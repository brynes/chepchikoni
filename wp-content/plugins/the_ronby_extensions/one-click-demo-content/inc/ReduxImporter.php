...
class ReduxImporter {
	...
	public static function import( $import_data ) {
		...
		if ( ! class_exists( 'ReduxFramework' ) ) {
			$error_message = esc_html__( 'The Redux plugin is not activated, so the Redux import was skipped!', 'ronby' );
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing Redux settings' , 'ronby' )
			);
		...
		foreach ( $import_data as $redux_item ) {
			...
			if ( isset( $redux_framework->args['opt_name'] ) ) {
				...
				$log_added = Helpers::append_to_file(
					sprintf( esc_html__( 'Redux settings import for: %s finished successfully!', 'ronby' ), $redux_item['option_name'] ),
					$log_file_path,
					esc_html__( 'Importing Redux settings' , 'ronby' )
				);
			}
			else {
				$error_message = sprintf( esc_html__( 'The Redux option name: %s, was not found in this WP site, so it was not imported!', 'ronby' ), $redux_item['option_name'] );
