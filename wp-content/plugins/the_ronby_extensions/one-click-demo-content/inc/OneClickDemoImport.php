...
class OneClickDemoImport {
	...
	public function create_plugin_page() {
		$plugin_page_setup = apply_filters( 'pt-ocdc/plugin_page_setup', array(
				...
				'menu_title'  => esc_html__( 'Import Demo' , 'ronby' ),
				'capability'  => 'import',
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_localize_script( 'ocdc-main-js', 'ocdc',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'ronby' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'ronby' ),
						'dialog_no'             => esc_html__( 'Cancel', 'ronby' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'ronby' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'ronby' ),
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'ronby' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdc/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'ronby' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'ronby' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'ronby' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdc/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks @FluentThemes! #OCDC https://fluentthemes.com/', 'ronby' );

				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sFluentThemes%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'ronby' ),
					'<div class="notice  notice-info"><p>',
					'<br>',
					'<strong><a href="https://fluentthemes.com/" target="_blank">',
					'</a></strong>',
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sOne Click Demo Import%4$s plugin, because it has done its job.%5$s', 'ronby' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'ronby' ),
