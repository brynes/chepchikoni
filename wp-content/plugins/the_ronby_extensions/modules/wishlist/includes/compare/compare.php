...
if ( is_admin() ) {
	require_once RONBY_WC_COMPARE_WISHLIST_PATH . 'includes/compare/settings.php';
}
...
function tm_woocompare_empty_message() {
	...
	$empty_text = get_option( 'tm_woocompare_empty_text', __( 'No products found to compare.', 'ronby' ) );
...
function tm_woocompare_process_ajax() {
	...
	if ( $is_page ) {
		...
		$json['compareList'] = ronby_woocompare_list_render_table();
	}
	if ( $is_widget ) {

		$json['widget'] = ronby_woocompare_list_render_widget();
	}
 ...
 */
function ronby_woocompare_list_render( $atts = array() ) {
	...
	$content[]              = ronby_woocompare_list_render_table( $atts );
	$content[]              = '</div>';
 ...
 */
function ronby_woocompare_list_render_widget() {
	...
	if ( $products->have_posts() ) {
		...
		$content[] = '</div>';
		$content[] = '<a href="' . tm_woocompare_get_page_link() . '" class="button btn btn-default compare_link_btn">' . get_option( 'tm_woocompare_page_btn_text', __( 'Compare products' , 'ronby' ) ) . '</a>';
		$content[] = '<button type="button" class="button btn btn-primary btn-danger tm-woocompare-empty">' . get_option( 'tm_woocompare_empty_btn_text', __( 'Empty compare' , 'ronby' ) ) . '</button>';
		$content[] = $tm_wc_compare_wishlist->get_loader();
	}
	wp_reset_query();
 ...
 */
function ronby_woocompare_list_render_table( $selected_attributes = array() ) {
