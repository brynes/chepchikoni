...
if ( is_admin() ) {
	require_once RONBY_WC_COMPARE_WISHLIST_PATH . 'includes/wishlist/settings.php';
}

if ( 'yes' !== get_option( 'ronby_woowishlist_enable' ) ) {
...
add_action( 'wp_enqueue_scripts', 'ronby_woowishlist_setup_plugin' );

add_action( 'wp_ajax_ronby_woowishlist_add', 'ronby_woowishlist_process_button_action' );
add_action( 'wp_ajax_nopriv_ronby_woowishlist_add', 'ronby_woowishlist_process_button_action' );

add_action( 'wp_ajax_ronby_woowishlist_remove', 'ronby_woowishlist_process_remove_button_action' );
add_action( 'wp_ajax_nopriv_ronby_woowishlist_remove', 'ronby_woowishlist_process_remove_button_action' );

add_action( 'wp_ajax_ronby_woowishlist_update', 'ronby_woowishlist_process_ajax' );
add_action( 'wp_ajax_nopriv_ronby_woowishlist_update', 'ronby_woowishlist_process_ajax' );
 ...
 */
function ronby_woowishlist_setup_plugin() {
 ...
 */
function ronby_woowishlist_get_list() {
 ...
 */
function ronby_woowishlist_set_list( $list ) {
 ...
 */
function ronby_woowishlist_get_page_link() {
	$page_id = intval( get_option( 'ronby_woowishlist_page', '' ) );
 ...
 * @since 1.0.0
 *
 * @action wp_ajax_ronby_woowishlist_add_to_list
 */
function ronby_woowishlist_process_button_action() {

	$id = filter_input( INPUT_POST, 'pid' );

	if ( ! wp_verify_nonce( filter_input( INPUT_POST, 'nonce' ), 'ronby_woowishlist' . $id ) ) {

		wp_send_json_error();
	}
	$button = json_decode( filter_input( INPUT_POST, 'single' ) ) ? ronby_woowishlist_page_button() : false;

	ronby_woowishlist_add( $id );
 ...
 */
function ronby_woowishlist_empty_message() {

	$empty_text = get_option( 'ronby_woowishlist_empty_text', __( 'No products added to wishlist.', 'ronby' ) );

	return apply_filters( 'ronby_woowishlist_empty_message', sprintf( '<p class="tm-woowishlist-empty">%s</p>', $empty_text ), $empty_text );
}
 ...
 * @since 1.0.0
 *
 * @action wp_ajax_ronby_woowishlist_update
 */
function ronby_woowishlist_process_ajax() {
	...
	if ( $is_page ) {
		...
		$json['wishList'] = ronby_woowishlist_render_table( $atts );
	}
	if ( $is_widget ) {

		$json['widget'] = ronby_woowishlist_render_widget();
	}
 ...
 * @since 1.0.0
 *
 * @action wp_ajax_ronby_woowishlist_remove
 */
function ronby_woowishlist_process_remove_button_action() {
	...
	ronby_woowishlist_remove( $id );

	ronby_woowishlist_process_ajax();
}
 ...
 */
function ronby_woowishlist_add( $id ) {
	...
	if( is_user_logged_in() ) {
		...
		$list   = ronby_woowishlist_get_list();
		$list[] = $id;
		$list   = array_unique( $list );

		ronby_woowishlist_set_list( $list );
	}
}
 ...
 */
function ronby_woowishlist_remove( $id ) {
 ...
 */
function ronby_woowishlist_get_products( $list ) {
 ...
 */
function ronby_woowishlist_render( $atts = array() ) {
	...
	$content[]              = ronby_woowishlist_render_table( $atts );
	$content[]              = '</div>';
 ...
 */
function ronby_woowishlist_render_widget() {

	$list = ronby_woowishlist_get_list();

	if ( empty( $list ) ) {

		return ronby_woowishlist_empty_message();
	}
	$templater = tm_wc_compare_wishlist_templater();
	$products  = ronby_woowishlist_get_products( $list );
	$template  = get_option( 'ronby_woowishlist_widget_template', 'widget.tmpl' );
	...
	if ( $products->have_posts() ) {
		...
		while ( $products->have_posts() ) {
			...
			$nonce        = wp_create_nonce( 'ronby_woowishlist' . $pid );
			$dismiss_icon = apply_filters( 'ronby_woowishlist_dismiss_icon', '<span class="dashicons dashicons-dismiss"></span>' );
		...
		$content[] = '</div>';
		$content[] = ronby_woowishlist_page_button( array( 'btn-default' ) );
		$content[] = $tm_wc_compare_wishlist->get_loader();
	}
	return implode( "\n", $content );
}
 ...
 */
function ronby_woowishlist_render_table( $atts = array() ) {
	...
	$templater = tm_wc_compare_wishlist_templater();
	$products  = ronby_woowishlist_get_products( $list );
	$template  = isset( $atts['template'] ) && ! empty( $atts['template'] ) ? $atts['template'] : get_option( 'ronby_woowishlist_page_template', 'page.tmpl' );
	$cols      = isset( $atts['cols'] )     && ! empty( $atts['cols'] )     ? $atts['cols']     : get_option( 'ronby_woowishlist_cols', '1' );
	$cols      = 4 < $cols                                                  ? 4                 : $cols;
	$template  = $templater->get_template_by_name( $template, 'tm-woowishlist' );
	...
	$html[] = '<div class="row">';
	$class  = apply_filters( 'ronby_woowishlist_column_class', 'col-lg-' . round( 12 / $cols ) . ' col-xs-12', $cols );
		...
		if ( ! empty( $session_list ) ) {
			...
			foreach ( $session_list as $product_id ) {
				...
				if( ! in_array( $product_id, $list ) ) {
					...
					ronby_woowishlist_add( $product_id );
				}
			}
			ronby_woowishlist_set_list( array() );
		}
	}
}