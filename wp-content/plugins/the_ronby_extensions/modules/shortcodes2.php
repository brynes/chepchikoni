...
function ronby_fitness_add_to_cart_button_two() {
       global $product;
       $classes = implode( ' ',  array(
           'button',
           'product_type_' . $product->get_type(),
       ...
       return apply_filters( 'woocommerce_loop_add_to_cart_link',
           sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="product_type_%s  ">'.esc_attr__('ADD TO CART','ronby').' <i class="fas fa-arrow-right"></i></a>',
               esc_url( $product->add_to_cart_url() ),
               esc_attr( $product->get_id() ),
               esc_attr( $product->get_sku() ),
               esc_attr( isset( $quantity ) ? $quantity : 1 ),
...
function ronby_bmi_weight_calculator_shortcode( $atts ) {
	...
	'cal_heading1' => '',
	'cal_heading2' => '',
	'weight_btn_label' => 'Compute Weight',
	'bmi_btn_label' => 'Compute BMI',
	), $atts ) );			
	...
	$output = '<div class="clac-slider '.esc_attr($unique_css_class).' page-header-3 row-xtra-space" '; if($images[0]){ $output.='style="background-image:url('.esc_url($images[0]).');background-size:cover;background-position:center top;background-attachment:fixed;"'; } $output.='>
					...
					<div class="row align-items-center">
						...
						<div class="col-auto">
							<div class="calculator-box color-inverse" '; if($calculator_box_bg_color){ $output.='style="background-color:'.esc_attr($calculator_box_bg_color).';"'; } $output.='>
								...
								'; if($cal_heading1){ $output.='
									<div class="box-subtitle">'.esc_attr($cal_heading1).'</div>
								'; } $output.='
								'; if($cal_heading2){ $output.='
									<div class="box-title">'.esc_attr($cal_heading2).'</div>
								'; } $output.='	
								</div>
								<div class="box-content">
									<div class="tabs-filter">
										<div class="tabs text-center mb-4">
											<span class="tab active" data-tab="bmi">'.esc_attr__('Bmi','ronby').'</span>
											<span class="tab" data-tab="weight">'.esc_attr__('Weight','ronby').'</span>
										</div>
										<div class="">
										<form class="bmi" method="POST">
											<div class="content-tab active" data-tab="bmi">
												<div class="form-group">
													<label for="feet" class="form-label">'.esc_attr__('Your height','ronby').'</label>
													<div class="row gutter-5">
														<div class="col">
															<input class="input-styled" type="text" placeholder="'.esc_attr__('Feet','ronby').'" id="feet" required>
														</div>
														<div class="col">
															<input class="input-styled" type="text" placeholder="'.esc_attr__('Inch','ronby').'" id="inch" required>
														</div>								</div>
												</div>
												<div class="form-group">
													<label for="weight" class="form-label">'.esc_attr__('Your weight','ronby').'</label>
													<input class="input-styled" type="text" placeholder="'.esc_attr__('Kg','ronby').'" id="weight" required>
												</div>
												<div class="form-group mt-3">
													<label class="custom-radio d-inline-flex mr-4">
														<input class="input-styled" type="radio" name="gender" value="female" required>
														<span class="checkmark"></span> '.esc_attr__('Female','ronby').'
													</label>
													<label class="custom-radio d-inline-flex">
														<input class="input-styled" type="radio" name="gender" value="male" required>
														<span class="checkmark"></span> '.esc_attr__('Male','ronby').'
													</label>
												</div>
												'; if($bmi_btn_label){ $output.='
												<div class="mt-4">
													<button class="button background-black color-white button-block rounded-capsule" type="submit">
														'.esc_attr($bmi_btn_label).'
													</button>
												</div>
												'; } $output.='
											...
											<div class="content-tab" data-tab="weight">
												<div class="form-group">
													<label for="feet2" class="form-label">'.esc_attr__('Your height','ronby').'</label>
													<div class="row gutter-5">
														<div class="col">
															<input class="input-styled" type="text" placeholder="'.esc_attr__('Feet','ronby').'" id="feet2" required>
														</div>
														<div class="col">
															<input class="input-styled" type="text" placeholder="'.esc_attr__('Inch','ronby').'" id="inch2" required>
														</div>
													</div>
												</div>
												<div class="form-group">
													<label for="weight2" class="form-label">'.esc_attr__('Your weight','ronby').'</label>
													<input class="input-styled" type="text" placeholder="'.esc_attr__('Kg','ronby').'" id="weight2">
												</div>
												<div class="form-group mt-3">
													<label class="custom-radio d-inline-flex mr-4">
												...
												'; if($weight_btn_label){ $output.='
												<div class="mt-4">
													<button class="button background-black color-white button-block rounded-capsule" type="submit">
														'.esc_attr($weight_btn_label).'
													</button>
												</div>
												'; } $output.='
...
jQuery('form.bmi').submit(function(e){
	...
	function bmiState(){
		if(calcBMI() < 18.5 ){
			return '".esc_attr__('Underweight','ronby')."';
		}
		if(calcBMI() > 18.5 && calcBMI() < 24.9 ){
			return '".esc_attr__('Normal weight','ronby')."';
		}

		if(calcBMI() > 25 && calcBMI() < 29.9 ){
			return '".esc_attr__('Overweight','ronby')."';
		}
		if(calcBMI() > 30 && calcBMI() < 34.9 ){
			return '".esc_attr__('Overweight class 1','ronby')."';
		}
		if(calcBMI() > 35 && calcBMI() < 39.9 ){
			return '".esc_attr__('Overweight class 2','ronby')."';
		}
		if(calcBMI() > 40){
			return '".esc_attr__('Overweight class 3','ronby')."';
		}
	}
	jQuery('.bmi_result').addClass('mt-4');
		resultDiv.html('".esc_attr__('Your Body Mass Index is','ronby')." '+ calcBMI() + '. ".esc_attr__('This is considered','ronby')." <b>' + bmiState() + '</b>');	
});
...
jQuery('form.weight-form').submit(function(e){
	...
	function weightState(){
		if(calcWEIGHT() < 137 ){
			return '".esc_attr__('This calculator for Adults Only','ronby')."';
		}
		if(calcWEIGHT() == 137 ){
			return '".esc_attr__('Your weight should be 28.5 - 34.9kg','ronby')."';
		}		
		if(calcWEIGHT() == 140 ){
			return '".esc_attr__('Your weight should be 30.8 - 38.1kg','ronby')."';
		}
		if(calcWEIGHT() == 142 ){
			return '".esc_attr__('Your weight should be 33.5 - 40.8kg','ronby')."';
		}
		if(calcWEIGHT() == 145 ){
			return '".esc_attr__('Your weight should be 35.8 - 43.9kg','ronby')."';
		}
		if(calcWEIGHT() == 147 ){
			return '".esc_attr__('Your weight should be 38.5 - 46.7kg','ronby')."';
		}
		if(calcWEIGHT() == 150 ){
			return '".esc_attr__('Your weight should be 40.8 - 49.9kg','ronby')."';
		}
		if(calcWEIGHT() == 152 ){
			return '".esc_attr__('Your weight should be 28.5 - 34.9kg','ronby')."';
		}
		if(calcWEIGHT() == 155 ){
			return '".esc_attr__('Your weight should be 45.8 - 55.8kg','ronby')."';
		}
		if(calcWEIGHT() == 157 ){
			return '".esc_attr__('Your weight should be 48.1 - 58.9kg','ronby')."';
		}
		if(calcWEIGHT() == 160 ){
			return '".esc_attr__('Your weight should be 50.8 - 61.6kg','ronby')."';
		}
		if(calcWEIGHT() == 163 ){
			return '".esc_attr__('Your weight should be 53 - 64.8kg','ronby')."';
		}
		if(calcWEIGHT() == 165 ){
			return '".esc_attr__('Your weight should be 55.3 - 68kg','ronby')."';
		}	
		if(calcWEIGHT() == 168 ){
			return '".esc_attr__('Your weight should be 58 - 70.7kg','ronby')."';
		}	
		if(calcWEIGHT() == 170 ){
			return '".esc_attr__('Your weight should be 60.3 - 73.9kg','ronby')."';
		}
		if(calcWEIGHT() == 173 ){
			return '".esc_attr__('Your weight should be 63 - 76.6kg','ronby')."';
		}
		if(calcWEIGHT() == 175 ){
			return '".esc_attr__('Your weight should be 65.3 - 79.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 178 ){
			return '".esc_attr__('Your weight should be 67.6 - 83kg','ronby')."';
		}
		if(calcWEIGHT() == 180 ){
			return '".esc_attr__('Your weight should be 70.3 - 85.7kg','ronby')."';
		}	
		if(calcWEIGHT() == 183 ){
			return '".esc_attr__('Your weight should be 72.6 - 88.9kg','ronby')."';
		}	
		if(calcWEIGHT() == 185 ){
			return '".esc_attr__('Your weight should be 75.3 - 91.6kg','ronby')."';
		}			
		if(calcWEIGHT() == 188 ){
			return '".esc_attr__('Your weight should be 77.5 - 94.8kg','ronby')."';
		}		
		if(calcWEIGHT() == 191 ){
			return '".esc_attr__('Your weight should be 79.8 - 98kg','ronby')."';
		}	
		if(calcWEIGHT() == 193 ){
			return '".esc_attr__('Your weight should be 82.5 - 100.6kg','ronby')."';
		}
		if(calcWEIGHT() == 195 ){
			return '".esc_attr__('Your weight should be 84.8 - 103.8kg','ronby')."';
		}
		if(calcWEIGHT() == 198 ){
			return '".esc_attr__('Your weight should be 87.5 - 106.5kg','ronby')."';
		}	
		if(calcWEIGHT() == 201 ){
			return '".esc_attr__('Your weight should be 89.8 - 109.7kg','ronby')."';
		}	
		if(calcWEIGHT() == 203 ){
			return '".esc_attr__('Your weight should be 92 - 112.9kg','ronby')."';
		}	
		if(calcWEIGHT() == 205 ){
			return '".esc_attr__('Your weight should be 94.8 - 115.6kg','ronby')."';
		}	
		if(calcWEIGHT() == 208 ){
			return '".esc_attr__('Your weight should be 97 - 118.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 210 ){
			return '".esc_attr__('Your weight should be 99.8 - 121.5kg','ronby')."';
		}
		if(calcWEIGHT() == 213 ){
			return '".esc_attr__('Your weight should be 102 - 124.7kg','ronby')."';
		}		
	}
	...
	function weightState(){
		...
		if(calcWEIGHT() == 140 ){
			return '".esc_attr__('Your weight should be 30.8 - 37.6kg','ronby')."';
		}
		if(calcWEIGHT() == 142 ){
			return '".esc_attr__('Your weight should be 32.6 - 39.9kg','ronby')."';
		}
		if(calcWEIGHT() == 145 ){
			return '".esc_attr__('Your weight should be 34.9 - 42.6kg','ronby')."';
		}
		if(calcWEIGHT() == 147 ){
			return '".esc_attr__('Your weight should be 36.4 - 44.9kg','ronby')."';
		}
		if(calcWEIGHT() == 150 ){
			return '".esc_attr__('Your weight should be 39 - 47.6kg','ronby')."';
		}
		if(calcWEIGHT() == 152 ){
			return '".esc_attr__('Your weight should be 40.8 - 49.9kg','ronby')."';
		}
		if(calcWEIGHT() == 155 ){
			return '".esc_attr__('Your weight should be 43.1 - 52.6kg','ronby')."';
		}
		if(calcWEIGHT() == 157 ){
			return '".esc_attr__('Your weight should be 44.9 - 54.9kg','ronby')."';
		}
		if(calcWEIGHT() == 160 ){
			return '".esc_attr__('Your weight should be 47.2 - 57.6kg','ronby')."';
		}
		if(calcWEIGHT() == 163 ){
			return '".esc_attr__('Your weight should be 49 - 59.9kg','ronby')."';
		}
		if(calcWEIGHT() == 165 ){
			return '".esc_attr__('Your weight should be 55.3 - 68kg','ronby')."';
		}	
		if(calcWEIGHT() == 168 ){
			return '".esc_attr__('Your weight should be 51.2 - 62.6kg','ronby')."';
		}	
		if(calcWEIGHT() == 170 ){
			return '".esc_attr__('Your weight should be 55.3 - 67.6kg','ronby')."';
		}
		if(calcWEIGHT() == 173 ){
			return '".esc_attr__('Your weight should be 57.1 - 69.8kg','ronby')."';
		}
		if(calcWEIGHT() == 175 ){
			return '".esc_attr__('Your weight should be 59.4 - 72.6kg','ronby')."';
		}	
		if(calcWEIGHT() == 178 ){
			return '".esc_attr__('Your weight should be 61.2 - 74.8kg','ronby')."';
		}
		if(calcWEIGHT() == 180 ){
			return '".esc_attr__('Your weight should be 63.5 - 77.5kg','ronby')."';
		}	
		if(calcWEIGHT() == 183 ){
			return '".esc_attr__('Your weight should be 65.3 - 79.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 185 ){
			return '".esc_attr__('Your weight should be 67.6 - 82.5kg','ronby')."';
		}			
		if(calcWEIGHT() == 188 ){
			return '".esc_attr__('Your weight should be 69.4 - 84.8kg','ronby')."';
		}		
		if(calcWEIGHT() == 191 ){
			return '".esc_attr__('Your weight should be 71.6 - 87.5kg','ronby')."';
		}	
		if(calcWEIGHT() == 193 ){
			return '".esc_attr__('Your weight should be 73.5 - 89.8kg','ronby')."';
		}
		if(calcWEIGHT() == 195 ){
			return '".esc_attr__('Your weight should be 75.7 - 92.5kg','ronby')."';
		}
		if(calcWEIGHT() == 198 ){
			return '".esc_attr__('Your weight should be 77.5 - 94.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 201 ){
			return '".esc_attr__('Your weight should be 79.8 - 97.5kg','ronby')."';
		}	
		if(calcWEIGHT() == 203 ){
			return '".esc_attr__('Your weight should be 81.6 - 99.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 205 ){
			return '".esc_attr__('Your weight should be 83.9 - 102.5kg','ronby')."';
		}	
		if(calcWEIGHT() == 208 ){
			return '".esc_attr__('Your weight should be 85.7 - 104.8kg','ronby')."';
		}	
		if(calcWEIGHT() == 210 ){
			return '".esc_attr__('Your weight should be 88 - 107.5kg','ronby')."';
		}
		if(calcWEIGHT() == 213 ){
			return '".esc_attr__('Your weight should be 89.8 - 109.7kg','ronby')."';
		}		
...
if($btn_text_color && $btn_bg_color){
	$output.='
	<style>
	.clac-slider.'.wp_trim_words($unique_css_class).' .button{
		color:'.esc_attr($btn_text_color).' !important;
		background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
}
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.clac-slider.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
}
	return $output;
...
function ronby_fitness_program_section_shortcode( $atts ) {
	...
	$output='<section class="section-join-us '.esc_attr($unique_css_class).' '; if($border_animation=="yes"){ $output.=' bdron'; } $output.='" '; if($bg_color){ $output.='style="background-color:'.esc_attr($bg_color).'"'; } $output.='>
				<div class="container" id="un343">
					<div class="section-content" '; if($images[0] || $padding_top || $padding_bottom){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');'; } if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).';'; } $output.='"'; } $output.='>
						...
						'; } $output.='
						<ul class="section-list-item d-flex flex-wrap align-items-center">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.section-join-us.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fitness_heading_section_one_shortcode( $atts ) {
	...
	$gallery = shortcode_atts(
    array(
        'heading_bgimage_glry'      =>  'heading_bgimage_glry',
    ), $atts );	
	$image_ids = explode(',',$gallery['heading_bgimage_glry']);
	
	foreach( $image_ids as $image_id ){
    $heading_bgimage = wp_get_attachment_image_src( $image_id, 'heading_bgimage_glry' );
	}
	...
	$output='<section class="fitness-heading-sec-one p-30-0-30 '; if($up_arrow_icon == 'style1'){ $output.='before-arrow-1 row-xtra-space'; } elseif($up_arrow_icon == 'style2'){ $output.='before-arrow-2 row-xtra-space'; } elseif($up_arrow_icon == 'style3'){ $output.='before-arrow-3 row-xtra-space'; } elseif($up_arrow_icon == 'style4'){ $output.='before-arrow-4 row-xtra-space'; } $output.='" '; if($padding_top || $padding_bottom){ $output.='style="'; if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).''; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).''; } $output.='"'; } $output.='>		
				<div class="section-header-style-6" '; if($heading_bgimage[0]){ $output.='style="background: url('.esc_url($heading_bgimage[0]).') center bottom no-repeat;"'; } $output.='>
				'; if($heading1){ $output.='
...
extract( shortcode_atts( array(
	...
	$output ='<section class="section-pricing-table '.esc_attr($unique_css_class).' p-30-0-30">
							<div class="plan-item style-1">
								<div class="item-header" '; if($images2[0]){ $output.='style="background-image:url('.esc_url($images2[0]).')"'; } $output.='>
									<div class="overlay d-flex flex-column justify-content-center" '; if($plan_header_overlay_color){ $output.='style="background-color:'.esc_attr($plan_header_overlay_color).'"'; } $output.='>
										...
										<div class="card-image">
											<img src="'.esc_url($images[0]).'" alt="'.esc_attr__('card-image','ronby').'">
										</div>
										'; } $output.='
									</div>
								</div>
								...
								<div class="item-content" '; if($images3[0]){ $output.='style="background-image:url('.esc_url($images3[0]).')"'; } $output.='>
									<div class="content-overlay">
										...
										$c++;
										$output.= '<li '; if($plan_feature_color){ $output.='style="color:'.esc_attr($plan_feature_color).'"'; } $output.='>'.strip_tags($feature).'</li>';
										} $output.='</ul>
										<a href="'.esc_url( $plan_btn_url).'" class="button button-secondary rounded-capsule button-block" '; if($plan_btn_text_color || $plan_btn_bg_color){ $output.='style="'; if($plan_btn_text_color){ $output.='color:'.esc_attr($plan_btn_text_color).';'; } if($plan_btn_bg_color){ $output.='background-color:'.esc_attr($plan_btn_bg_color).''; } $output.='"'; } $output.='>
											'.esc_attr($plan_btn_name).' <i class="fas fa-angle-right ml-5px"></i>
										</a>
									</div>
								</div>	
								'; } $output.='								
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.section-pricing-table.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
extract( shortcode_atts( array(
	...
	'tooltip_anchor_color' => '',	
	), $atts ) );
	$content2='';
	if(!empty($features)){
      $res2 = '<div class="filter-nav-2 mb-2">	  
				  <ul class="list-unstyled">
				  '; if($showall){ $res2.='
					<li  class="showall actv">
						'.esc_attr($showall).'
				  </li>';
				  }
	  ...
	  $c++;
      $res2 .= '<li class="tab" data-tab="'.$feature.'">';
				
	  $res2 .=''.htmlspecialchars_decode($feature).'</li>';
      }
      $res2 .= '</ul></div>';
      $content2= $res2;
    }	
...
if(isset(ronby_get_option('ronby_web_global_color_primary')['rgba']) && ronby_get_option('ronby_web_global_color_primary')['rgba'] !=''){
	$anchor_color=ronby_get_option('ronby_web_global_color_primary')['rgba'];
} else {
	if($tooltip_anchor_color){
	$anchor_color=$tooltip_anchor_color;	
	} else {
	$anchor_color='#fff';	
	}
}	
	$output ='<section class="section-timetable p-30-0-30 fitness-section-timetable  '.esc_attr($unique_css_class).'">
    <div class="container">
						<div class="tabs-filter tabs-filter-style-1">
							'; if($content2){ $output.=$content2; } $output.='
	...
	'col1_tooltip_title'.$c.'' => '',	
	'col1_tooltip_subtitle'.$c.'' => '',	
	'col1_tooltip_desc'.$c.'' => '',	
	'col1_tooltip_btn_label'.$c.'' => '',	
	'col1_tooltip_btn_url'.$c.'' => '',	
	
	'col2_tooltip_title'.$c.'' => '',	
	'col2_tooltip_subtitle'.$c.'' => '',	
	'col2_tooltip_desc'.$c.'' => '',	
	'col2_tooltip_btn_label'.$c.'' => '',	
	'col2_tooltip_btn_url'.$c.'' => '',		
	
	'col3_tooltip_title'.$c.'' => '',	
	'col3_tooltip_subtitle'.$c.'' => '',	
	'col3_tooltip_desc'.$c.'' => '',	
	'col3_tooltip_btn_label'.$c.'' => '',	
	'col3_tooltip_btn_url'.$c.'' => '',	
	
	'col4_tooltip_title'.$c.'' => '',	
	'col4_tooltip_subtitle'.$c.'' => '',	
	'col4_tooltip_desc'.$c.'' => '',	
	'col4_tooltip_btn_label'.$c.'' => '',	
	'col4_tooltip_btn_url'.$c.'' => '',	
	
	'col5_tooltip_title'.$c.'' => '',	
	'col5_tooltip_subtitle'.$c.'' => '',	
	'col5_tooltip_desc'.$c.'' => '',	
	'col5_tooltip_btn_label'.$c.'' => '',	
	'col5_tooltip_btn_url'.$c.'' => '',	
	
	'col6_tooltip_title'.$c.'' => '',	
	'col6_tooltip_subtitle'.$c.'' => '',	
	'col6_tooltip_desc'.$c.'' => '',	
	'col6_tooltip_btn_label'.$c.'' => '',	
	'col6_tooltip_btn_url'.$c.'' => '',	
	
	'col7_tooltip_title'.$c.'' => '',	
	'col7_tooltip_subtitle'.$c.'' => '',	
	'col7_tooltip_desc'.$c.'' => '',	
	'col7_tooltip_btn_label'.$c.'' => '',	
	'col7_tooltip_btn_url'.$c.'' => '',	
	
	'col8_tooltip_title'.$c.'' => '',	
	'col8_tooltip_subtitle'.$c.'' => '',	
	'col8_tooltip_desc'.$c.'' => '',	
	'col8_tooltip_btn_label'.$c.'' => '',	
	'col8_tooltip_btn_url'.$c.'' => '',	
    ),$atts);
	...
	$col1_feature  =$b['col1_feature'.$c.''];	
	$col2_feature  =$b['col2_feature'.$c.''];	
	$col3_feature  =$b['col3_feature'.$c.''];	
	$col4_feature  =$b['col4_feature'.$c.''];	
	$col5_feature  =$b['col5_feature'.$c.''];	
	$col6_feature  =$b['col6_feature'.$c.''];	
	$col7_feature  =$b['col7_feature'.$c.''];	
	$col8_feature  =$b['col8_feature'.$c.''];	
	...
	$col1_tooltip_content= 'col1_tooltip_content'.$c;
	$col2_tooltip_content= 'col2_tooltip_content'.$c;
	$col3_tooltip_content= 'col3_tooltip_content'.$c;
	$col4_tooltip_content= 'col4_tooltip_content'.$c;
	$col5_tooltip_content= 'col5_tooltip_content'.$c;
	$col6_tooltip_content= 'col6_tooltip_content'.$c;
	$col7_tooltip_content= 'col7_tooltip_content'.$c;
	$col8_tooltip_content= 'col8_tooltip_content'.$c;
	
	$col1_tooltip_title= $b['col1_tooltip_title'.$c.''];
	$col1_tooltip_subtitle= $b['col1_tooltip_subtitle'.$c.''];
	$col1_tooltip_desc= $b['col1_tooltip_desc'.$c.''];
	$col1_tooltip_btn_label= $b['col1_tooltip_btn_label'.$c.''];
	$col1_tooltip_btn_url=  $b['col1_tooltip_btn_url'.$c.''];

	$col2_tooltip_title= $b['col2_tooltip_title'.$c.''];
	$col2_tooltip_subtitle= $b['col2_tooltip_subtitle'.$c.''];
	$col2_tooltip_desc= $b['col2_tooltip_desc'.$c.''];
	$col2_tooltip_btn_label= $b['col2_tooltip_btn_label'.$c.''];
	$col2_tooltip_btn_url=  $b['col2_tooltip_btn_url'.$c.''];

	$col3_tooltip_title= $b['col3_tooltip_title'.$c.''];
	$col3_tooltip_subtitle= $b['col3_tooltip_subtitle'.$c.''];
	$col3_tooltip_desc= $b['col3_tooltip_desc'.$c.''];
	$col3_tooltip_btn_label= $b['col3_tooltip_btn_label'.$c.''];
	$col3_tooltip_btn_url=  $b['col3_tooltip_btn_url'.$c.''];

	$col4_tooltip_title= $b['col4_tooltip_title'.$c.''];
	$col4_tooltip_subtitle= $b['col4_tooltip_subtitle'.$c.''];
	$col4_tooltip_desc= $b['col4_tooltip_desc'.$c.''];
	$col4_tooltip_btn_label= $b['col4_tooltip_btn_label'.$c.''];
	$col4_tooltip_btn_url=  $b['col4_tooltip_btn_url'.$c.''];

	$col5_tooltip_title= $b['col5_tooltip_title'.$c.''];
	$col5_tooltip_subtitle= $b['col5_tooltip_subtitle'.$c.''];
	$col5_tooltip_desc= $b['col5_tooltip_desc'.$c.''];
	$col5_tooltip_btn_label= $b['col5_tooltip_btn_label'.$c.''];
	$col5_tooltip_btn_url=  $b['col5_tooltip_btn_url'.$c.''];

	$col6_tooltip_title= $b['col6_tooltip_title'.$c.''];
	$col6_tooltip_subtitle= $b['col6_tooltip_subtitle'.$c.''];
	$col6_tooltip_desc= $b['col6_tooltip_desc'.$c.''];
	$col6_tooltip_btn_label= $b['col6_tooltip_btn_label'.$c.''];
	$col6_tooltip_btn_url=  $b['col6_tooltip_btn_url'.$c.''];

	$col7_tooltip_title= $b['col7_tooltip_title'.$c.''];
	$col7_tooltip_subtitle= $b['col7_tooltip_subtitle'.$c.''];
	$col7_tooltip_desc= $b['col7_tooltip_desc'.$c.''];
	$col7_tooltip_btn_label= $b['col7_tooltip_btn_label'.$c.''];
	$col7_tooltip_btn_url=  $b['col7_tooltip_btn_url'.$c.''];

	$col8_tooltip_title= $b['col8_tooltip_title'.$c.''];
	$col8_tooltip_subtitle= $b['col8_tooltip_subtitle'.$c.''];
	$col8_tooltip_desc= $b['col8_tooltip_desc'.$c.''];
	$col8_tooltip_btn_label= $b['col8_tooltip_btn_label'.$c.''];
	$col8_tooltip_btn_url=  $b['col8_tooltip_btn_url'.$c.''];
	
    $output.='<tr>  			  
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col1_tooltip_content).'" '; if($col1_text_color || $col1_bg_color){ $output.='style="'; if($col1_text_color){ $output.='color:'.esc_attr($col1_text_color).';'; } if($col1_bg_color){ $output.='background-color:'.esc_attr($col1_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all d-block" data-tab="'.esc_attr( $col1_feature ).'">'; if($col1_text){ $output.=''.wp_kses($col1_text, $allowed_html_array).''; } $output.='				'; if($col1_tooltip_title || $col1_tooltip_subtitle || $col1_tooltip_desc || $col1_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col1_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col1_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col1_tooltip_title).'</div>
							'; } $output.='
							'; if($col1_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col1_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col1_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col1_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col1_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col1_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col1_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col2_tooltip_content).'" '; if($col2_text_color || $col2_bg_color){ $output.='style="'; if($col2_text_color){ $output.='color:'.esc_attr($col2_text_color).';'; } if($col2_bg_color){ $output.='background-color:'.esc_attr($col2_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active  " data-tab="'.esc_attr( $col2_feature ).'">'; if($column_title_2){ $output.=''.wp_kses($col2_text, $allowed_html_array).''; } $output.='				
				'; if($col2_tooltip_title || $col2_tooltip_subtitle || $col2_tooltip_desc || $col2_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col2_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col2_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col2_tooltip_title).'</div>
							'; } $output.='
							'; if($col2_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col2_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col2_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col2_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col2_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col2_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col2_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col3_tooltip_content).'" '; if($col3_text_color || $col3_bg_color){ $output.='style="'; if($col3_text_color){ $output.='color:'.esc_attr($col3_text_color).';'; } if($col3_bg_color){ $output.='background-color:'.esc_attr($col3_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all " data-tab="'.esc_attr( $col3_feature ).'">'; if($column_title_3){ $output.=''.wp_kses($col3_text, $allowed_html_array).''; } $output.='				
				'; if($col3_tooltip_title || $col3_tooltip_subtitle || $col3_tooltip_desc || $col3_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col3_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col3_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col3_tooltip_title).'</div>
							'; } $output.='
							'; if($col3_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col3_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col3_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col3_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col3_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col3_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col3_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col4_tooltip_content).'" '; if($col4_text_color || $col4_bg_color){ $output.='style="'; if($col4_text_color){ $output.='color:'.esc_attr($col4_text_color).';'; } if($col4_bg_color){ $output.='background-color:'.esc_attr($col4_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all " data-tab="'.esc_attr( $col4_feature ).'">'; if($column_title_4){ $output.=''.wp_kses($col4_text, $allowed_html_array).''; } $output.='				
				'; if($col4_tooltip_title || $col4_tooltip_subtitle || $col4_tooltip_desc || $col4_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col4_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col4_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col4_tooltip_title).'</div>
							'; } $output.='
							'; if($col4_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col4_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col4_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col4_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col4_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col4_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col4_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col5_tooltip_content).'" '; if($col5_text_color || $col5_bg_color){ $output.='style="'; if($col5_text_color){ $output.='color:'.esc_attr($col5_text_color).';'; } if($col5_bg_color){ $output.='background-color:'.esc_attr($col5_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all  " data-tab="'.esc_attr( $col5_feature ).'">';  if($column_title_5){ $output.=''.wp_kses($col5_text, $allowed_html_array).''; } $output.='				
				'; if($col5_tooltip_title || $col5_tooltip_subtitle || $col5_tooltip_desc || $col5_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col5_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col5_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col5_tooltip_title).'</div>
							'; } $output.='
							'; if($col5_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col5_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col5_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col5_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col5_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col5_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col5_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col6_tooltip_content).'" '; if($col6_text_color || $col6_bg_color){ $output.='style="'; if($col6_text_color){ $output.='color:'.esc_attr($col6_text_color).';'; } if($col6_bg_color){ $output.='background-color:'.esc_attr($col6_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all  " data-tab="'.esc_attr( $col6_feature ).'">';  if($column_title_6){ $output.=''.wp_kses($col6_text, $allowed_html_array).''; } $output.='
				'; if($col6_tooltip_title || $col6_tooltip_subtitle || $col6_tooltip_desc || $col6_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col6_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col6_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col6_tooltip_title).'</div>
							'; } $output.='
							'; if($col6_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col6_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col6_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col6_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col6_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col6_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col6_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
                ...
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col7_tooltip_content).'" '; if($col7_text_color || $col7_bg_color){ $output.='style="'; if($col7_text_color){ $output.='color:'.esc_attr($col7_text_color).';'; } if($col7_bg_color){ $output.='background-color:'.esc_attr($col7_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all " data-tab="'.esc_attr( $col7_feature ).'">';  if($column_title_7){ $output.=''.wp_kses($col7_text, $allowed_html_array).''; } $output.='				
				'; if($col7_tooltip_title || $col7_tooltip_subtitle || $col7_tooltip_desc || $col7_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col7_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col7_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col7_tooltip_title).'</div>
							'; } $output.='
							'; if($col7_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col7_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col7_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col7_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col7_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col7_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col7_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
				...
				'; if($column_title_8){ $output.='
                <td class="tooltipp" data-tooltip-content="#'.esc_attr($col8_tooltip_content).'" '; if($col8_text_color || $col8_bg_color){ $output.='style="'; if($col8_text_color){ $output.='color:'.esc_attr($col8_text_color).';'; } if($col8_bg_color){ $output.='background-color:'.esc_attr($col8_bg_color).''; } $output.='"'; } $output.='><div class="content-tab active all " data-tab="'.esc_attr( $col8_feature).'">'; if($column_title_8){ $output.=''.wp_kses($col8_text, $allowed_html_array).''; } $output.='				'; if($col8_tooltip_title || $col8_tooltip_subtitle || $col8_tooltip_desc || $col8_tooltip_btn_label){ $output.='
				<div class="tooltip_templates">
					<span id="'.esc_attr($col8_tooltip_content).'">
						<div class="container">
							<div class="row mb-2">
							'; if($col8_tooltip_title){ $output.='
								<div class="col-md-6 text-left">'.esc_attr($col8_tooltip_title).'</div>
							'; } $output.='
							'; if($col8_tooltip_subtitle){ $output.='
								<div class="col-md-6 text-right">'.esc_attr($col8_tooltip_subtitle).'</div>
							'; } $output.='	
							</div>
							'; if($col8_tooltip_desc){ $output.='
							<div class="row mb-2">
								<div class="col-md-12">'.$col8_tooltip_desc.'</div>
							</div>
							'; } $output.='
							'; if($col8_tooltip_btn_label){ $output.='
							<div class="row mb-2">
								<div class="col-md-12"><a href="'.esc_url($col8_tooltip_btn_url).'" '; if($anchor_color){ $output.='style="color:'.esc_attr($anchor_color).'"'; } $output.='>'.esc_attr($col8_tooltip_btn_label).'</a></div>
							</div>
							'; } $output.='
						</div>
					</span>
...
jQuery('.showall').on('click',function(){
    ...
    jQuery('.tabs-filter-content .content-tab').addClass('active');
});
jQuery('.tab').on('click',function(){
    jQuery('.showall').removeClass('actv');	
});
});
...
if($active_tab_bg_color && $active_tab_text_color){
	$output.='
	<style>
	.fitness-section-timetable.'.wp_trim_words($unique_css_class).'   .showall.actv,.fitness-section-timetable.'.wp_trim_words($unique_css_class).'  .tab.active{
		color:'.esc_attr($active_tab_text_color).' !important;
		background-color:'.esc_attr($active_tab_bg_color).';
	}	
	</style>';
}
...
function ronby_fitness_woo_products_section_shortcode( $atts ) {
	...
	$output='<section class="section-feature-products '.esc_attr($unique_css_class).' p-30-0-30 fitness-section-feature-products">
	<div class="container">
				<div class="row">';
...
if($badge_bg_before && $badge_bg_after){
	$output.='
	<style>
	.fitness-section-feature-products.'.wp_trim_words($unique_css_class).'  .ribbon-wrap::before{
		background-color:'.esc_attr($badge_bg_before).';
	}	
	.fitness-section-feature-products.'.wp_trim_words($unique_css_class).'  .ribbon-wrap::after{
		background-color:'.esc_attr($badge_bg_after).';
	}	
	</style>';
}
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.fitness-section-feature-products.'.wp_trim_words($unique_css_class).'  .item-buttons a:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fitness_trainers_profile_shortcode( $atts ) {
	...
	$output='<section class="section-teams '.esc_attr($unique_css_class).' p-30-0-30 fitness-section-teams '; if($up_arrow_icon == 'style1'){ $output.='before-arrow-1 row-xtra-space'; } elseif($up_arrow_icon == 'style2'){ $output.='before-arrow-2 row-xtra-space'; } elseif($up_arrow_icon == 'style3'){ $output.='before-arrow-3 row-xtra-space'; } elseif($up_arrow_icon == 'style4'){ $output.='before-arrow-4 row-xtra-space'; } $output.='">
				<div class="container">
					...
					'; if($lead_trainer_img_url[0]){ $output.='
						<article class="detailed-team-member-item-2 thumbnail animate-zoom">
							...
							<img src="'.esc_url($lead_trainer_img_url[0]).'" alt="'.esc_attr__('lead-trainer-image','ronby').'">
						...
						'; if($trainer_img_url){ $output.='
							...
							<div class="thumbnail animate-zoom">
								<a href="'.esc_url($trainer_link_url).'">
									<img src="'.esc_url($trainer_img_url).'" alt="'.esc_attr__('trainer-profile-image','ronby').'">
								</a>
...
if($social_icon_color && $social_icon_bg_color){
	$output.='
	<style>
	.fitness-section-teams.'.wp_trim_words($unique_css_class).' .member-socials ul li a{
		color:'.esc_attr($social_icon_color).' !important;
		background-color:'.esc_attr($social_icon_bg_color).' !important;
	}	
	</style>';
}
if($social_icon_hover_color && $social_icon_hover_bg_color){
	$output.='
	<style>
	.fitness-section-teams.'.wp_trim_words($unique_css_class).' .member-socials ul li a:hover{
		color:'.esc_attr($social_icon_hover_color).' !important;
		background-color:'.esc_attr($social_icon_hover_bg_color).' !important;
	}	
	</style>';
}			
	return $output;
...
function ronby_fitness_blog_section_shortcode( $atts ) {
	...
	$output ='<section class="ronby-fitness-blog-section '.esc_attr($unique_css_class).'"><div class="container">'; 
	$counter = 1;
	if ($wp_query->have_posts()) :  while ($wp_query->have_posts()) : $wp_query->the_post();
...
if($btn_text_color && $btn_bg_color){
	$output.='
	<style>
	.ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .button{
		color:'.esc_attr($btn_text_color).' !important;
		background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
}
if($meta_color || $title_color || $desc_color ){
	$output.='
	<style>
	'; if($meta_color){ $output.='
	.ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .blog-post-item-4 .post-date, .ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .blog-post-item-4 .post-stats{
		color:'.esc_attr($meta_color).' !important;
	}
	'; } $output.='	
	'; if($title_color){ $output.='
	.ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .blog-post-item-4 .post-title{
		color:'.esc_attr($title_color).' ;
	}
	'; } $output.='
	
	'; if($desc_color){ $output.='
	.ronby-fitness-blog-section.'.wp_trim_words($unique_css_class).' .blog-post-item-4 .post-excerpt{
...
function ronby_fitness_brand_slider_shortcode( $atts ) {
	...
	$output.='<div class="item">
				<img src="'.esc_url($images[$c]).'" alt="'.esc_attr__('slider-image','ronby').'">
			</div>';
	}$c++;
	$output.='</div>
			</div>
...
function ronby_fitness_class_details_shortcode( $atts ) {
	...
	$output='<section class="class-detail p-30-0-30 '.esc_attr($unique_css_class).'">
				<div class="container">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.class-detail.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
						...
						'; } $output.='	
							<div class="row">
								...
								<div class="col-lg-5">
									...
									'; if($email){ $output.='
										<div class="mb-2">
											<span class="color-primary" '; if($information_title_color){ $output.='style="color:'.esc_attr($information_title_color).'"'; } $output.='>'.esc_attr__('Email:','ronby').'</span> <span '; if($information_details_color){ $output.='style="color:'.esc_attr($information_details_color).'"'; } $output.='>'.esc_attr($email).'</span>
										</div>
									'; } $output.='
									'; if($phone){ $output.='
										<div class="mb-2">
											<span class="color-primary" '; if($information_title_color){ $output.='style="color:'.esc_attr($information_title_color).'"'; } $output.='>'.esc_attr__('Phone:','ronby').' </span> <span '; if($information_details_color){ $output.='style="color:'.esc_attr($information_details_color).'"'; } $output.='>'.esc_attr($phone).'</span>
										</div>
									'; } $output.='	
									'; if($fax){ $output.='
										<div class="mb-2">
											<span class="color-primary" '; if($information_title_color){ $output.='style="color:'.esc_attr($information_title_color).'"'; } $output.='>'.esc_attr__('Fax:','ronby').'</span> <span '; if($information_details_color){ $output.='style="color:'.esc_attr($information_details_color).'"'; } $output.='>'.esc_attr($fax).'</span>
										</div>
									'; } $output.='
									'; if($address){ $output.='
										<p class="m-30-0-0">
...
function ronby_fitness_contact_form_shortcode( $atts ) {
	...
	$output='<section class="fitness-contact-form '.esc_attr($unique_css_class).' p-30-0-30">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-12 col-xl-10">
					...
					<div class="contact-form pb-70">
						<form  method="POST" id="fitness_contact_form">
							<div class="row align-items-center">
								<div class="col-lg-4">
									<div class="form-group">
										<input class="input-styled" type="text" name="name" id="sendername" placeholder="'.esc_attr($name_field_placeholder).'" '; if($input_box_bg){ $output.='style="background-color:'.esc_attr($input_box_bg).'"'; } $output.=' required>
									</div>
								</div>
								<div class="col-lg-4">
									<div class="form-group">
										<input class="input-styled" type="email" name="email" id="senderemail" placeholder="'.esc_attr($email_field_placeholder).'" '; if($input_box_bg){ $output.='style="background-color:'.esc_attr($input_box_bg).'"'; } $output.=' required>
									</div>
								</div>
								<div class="col-lg-4">
									<div class="form-group">
										<input class="input-styled" type="text" name="contact_issue" id="senderproblem" placeholder="'.esc_attr($problem_field_placeholder).'" '; if($input_box_bg){ $output.='style="background-color:'.esc_attr($input_box_bg).'"'; } $output.='>
									</div>
								</div>
								'; if($wanttoselect=="yes"){
						...
						$output .='</div></div>';
							...
							<div class="form-group">
								<textarea class="input-styled" name="message" id="sendermessage" rows="10" placeholder="'.esc_attr($message_field_placeholder).'" required  '; if($input_box_bg){ $output.='style="background-color:'.esc_attr($input_box_bg).'"'; } $output.='></textarea>
							</div>
							<div class="form-group py-15px text-center">
								<button class="button rounded-capsule comment-submit" type="submit" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader','ronby').'"/></button>
	...
	$output.='
	<style>
	.fitness-contact-form.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
  ...
  $output.='<div class="page-header-1 row-xtra-space" id="page-header" '; if($images[0]){ $output.='style="background-image:url('.esc_url($images[0]).')"'; } $output.='>
    <div class="overlay" '; if($overlay_color){ $output.='style="background-color:'.esc_attr($overlay_color).'"'; } $output.='>       
      <div class="container">
		...
		'; if((ronby_get_option('blog_page_header_sec_title_one'))) { $output.='
		...
		'; if((ronby_get_option('blog_page_header_sec_title_two'))) { $output.='	
...
function ronby_fitness_team_details_shortcode( $atts ) {
	...
	$output='
		<div class="team-details-style '.esc_attr($unique_css_class).' p-30-0-30">	
			<div class="container">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.team-details-style.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_pricing_badge_shortcode( $atts ) {
	...
	<!--end badge -->    
	'; if($images[0]){ $output.='
	<img src="'.esc_url($images[0]).'" alt="'.esc_attr__('featured-image','ronby').'" class="img-responsive"/> 
	'; } $output.='
	</div></div></section>';
				...
				<div class="team-detail-header">
					...
					'; if($chef_img[0]){ $output.='
						<div class="col-lg-4 text-right" '; if($master_chef_img_bg_color){ $output.='style=background-color:'.esc_attr($master_chef_img_bg_color).''; } $output.='>						
							<img class="mt-m-160" src="'.esc_url($chef_img[0]).'" alt="'.esc_attr__('master-chef-featured-image','ronby').'">
						</div>
					'; } $output.='	
...
function ronby_restaurant_products_tab_section_shortcode( $atts ) {
	...
	if(!empty($features)){
      $output = '<div id="filters" class="filter-nav-3">
				  <ul class="no-style items-inline-block text-center pl-0">
				  '; if($showall){ $output.='
...
function ronby_restaurant_video_section_shortcode( $atts ) {
	...
	$output='<section>
			...
			'; if($bgimage[0]){ $output.='
				<img class="w-100-percent" src="'.esc_url($bgimage[0]).'" alt="'.esc_attr__('background-image','ronby').'">
			'; } $output.='	
				<div class="video-overlay d-flex align-items-center justify-content-center">
					<div class="section-header-style-15 mb-0 bg-none pb-0">
					'; if($heading1){ $output.='
						<h4 class="section-header-sub-title color-primary" '; if($heading1_color){ $output.='style="color:'.esc_attr($heading1_color).'"'; } $output.='>'.esc_attr($heading1).'</h4>
					'; } $output.='
					'; if($heading2){ $output.='
				...
				'; } $output.='	
					<div class="contact-form pb-70">
						...
						$output .='</div></div>';
							...
							<div class="form-group py-15px text-center">
								<button class="button  comment-submit button-primary" type="submit" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader','ronby').'"/></button>
...
foreach( $explode_bg_img as $explode_bg_img_id ){
	...
	$output='<section class="section-side-image clearfix">
		  ...
		  '; if($get_bg_image[0]){ $output.='
	        <div class="background-imgholder" style="background:url('.esc_url($get_bg_image[0]).');"><img class="nodisplay-image" src="'.esc_url($get_bg_image[0]).'" alt="'.esc_attr__('background-image').'"> </div>
		  '; } $output.='
	      </div>
...
function ronby_business_our_service_section_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 business_our_service_section">
			<div class="container">
				<div class="row">
...
function ronby_business_company_ranking_progress_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 business_company_ranking_progress">
			<div class="container">
				<div class="row">
...
function ronby_construction_feature_box_shortcode( $atts ) {
	...
	$output='<article class="article-with-overlay-5 '.esc_attr($unique_css_class).' p-30-0-30 mb-0" '; if($p_top || $p_bottom){ $output.='style="'; if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).''; } $output.='"'; } $output.='>
				'; if($get_image[0]){ $output.='
				<div class="thumbnail">
				...
				<a href="'.esc_url($link_to_url).'">
					...
					'; } $output.='	
					'; if($desc){ $output.='
						<div class="item-excerpt  d-xl-block" '; if($desc_color){ $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>
							'.$desc.'
						</div>
					'; } $output.='	
					</div>
...
if($hover_icon || $hover_overlay || $hover_title || $hover_cat || $hover_desc || $hover_arrow){
	$output.='
	<style>
@media (min-width: 992px) {';
if($hover_icon){ $output.='
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .icon{
         color: '.esc_attr($hover_icon).' !important;
    }
'; } if($hover_overlay){ $output.='
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .overlay {
         background: '.esc_attr($hover_overlay).' !important;
    }
'; } if($hover_title){ $output.='	
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .overlay .item-title {
         color: '.esc_attr($hover_title).' !important;
    }
'; } if($hover_cat){ $output.='	
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .overlay .item-description {
         color: '.esc_attr($hover_cat).' !important;
    }
'; } if($hover_desc){ $output.='		
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .overlay .item-excerpt {
         color: '.esc_attr($hover_desc).' !important;
    }
'; } if($hover_arrow){ $output.='	
     .article-with-overlay-5.'.wp_trim_words($unique_css_class).':hover .arrow {
         background: '.esc_attr($hover_arrow).' !important;
    }
'; } $output.='	
}	
	</style>';
...
function ronby_construction_button_shortcode( $atts ) {
	...
	$output='<div class="text-center button-wrapper '.esc_attr($unique_css_class).'">
			'; if($btn_label){ $output.='
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.button-wrapper.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.section-we-provide-services.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
		border-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
}	
...
function ronby_construction_feature_box_two_shortcode( $atts ) {
	...
	$output='<article class="blog-post-item-3 '.esc_attr($unique_css_class).' p-30-0-30 mb-0">
				<div class="thumbnail animate-zoom">
				'; if($get_image[0]){ $output.='
					<a href="'.esc_url($link_to_url).'">
						<img src="'.esc_url($get_image[0]).'" alt="'.esc_attr__('featured-image','ronby').'">
...
if($btn_hover_color || $btn_arrow_bg){
	$output.='
	<style>
	'; if($btn_hover_color){ $output.='
	.blog-post-item-3.'.wp_trim_words($unique_css_class).' a:hover{
		color:'.esc_attr($btn_hover_color).' !important;
	}
	'; } if($btn_arrow_bg){ $output.='	
	.blog-post-item-3.'.wp_trim_words($unique_css_class).' a::after{
		background-color:'.esc_attr($btn_arrow_bg).' !important;
	}	
	'; } $output.='
	</style>';
}		
...
function ronby_construction_projects_tab_sec_shortcode( $atts ) {
	...
	$out = '<section class="construction-our-projects '.esc_attr($unique_css_class).' p-30-0-30">
...
if($active_tab_color){
	$out.='
	<style>
	.construction-our-projects.'.wp_trim_words($unique_css_class).' .active-color-primary.active{
		color:'.esc_attr($active_tab_color).' !important;
	}	
	.construction-our-projects.'.wp_trim_words($unique_css_class).' .active-color-primary:hover{
		color:'.esc_attr($active_tab_color).' !important;
	}	
	</style>';
} 
if($box_hover_color){
	$out.='
	<style>
	.construction-our-projects.'.wp_trim_words($unique_css_class).' .article-with-overlay-4:hover .overlay{
		background-color:'.esc_attr($box_hover_color).' !important;
	}		
	</style>';
}

if($hover_icon_color || $hover_icon_bg || $hover_text_color){
	$out.='
	<style>
	'; if($hover_icon_color || $hover_icon_bg){$out.='
	.construction-our-projects.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .icon{
		color:'.esc_attr($hover_icon_color).' !important;
		background-color:'.esc_attr($hover_icon_bg).' !important;
	}
	'; } if($hover_text_color){ $out.='
	.construction-our-projects.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .post-title{
...
function ronby_construction_faq_shortcode( $atts ) {
	...
	$output='<section '; if($padding_top || $padding_bottom || $padding_left || $padding_right){ $output.='style="'; if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).';'; } if($padding_left){ $output.='padding-left:'.esc_attr($padding_left).';'; } if($padding_right){ $output.='padding-right:'.esc_attr($padding_right).';'; } $output.='"'; } $output.=' class="construction-faq '.esc_attr($unique_css_class).'">
	...
	if($icon_color){
		$output.='
		<style>
		.construction-faq.'.wp_trim_words($unique_css_class).' .item-title::before{
...
function ronby_construction_contact_form_shortcode( $atts ) {
	...
	$output='<section class="construction-contact-form '.esc_attr($unique_css_class).' section-right section-form-box m-0" '; if($get_image[0]){ $output.='style="background-image:url('.esc_url($get_image[0]).');background-size: cover;background-position: center center;"'; } $output.='>
		<div class="overlay pl-0" '; if($overlay_bg){ $output.='style="background-color:'.esc_attr($overlay_bg).'"'; } $output.='>
				...
				'; } } $output.='	
					<div class="form-style-4">
						...
						'; } $output.='	
							<div class="pt-4">
								<button class="button button-primary rounded" type="submit" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader','ronby').'"/></button>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.construction-contact-form.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
		border-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
...
function ronby_construction_blog_section_shortcode( $atts ) {
	...
	$wp_query= null;			
	$wp_query = new WP_Query($args);	
	$output='<section class="p-30-0-30 business-blog-section-element '.esc_attr($unique_css_class).'">
...
if($blog_title_color || $blog_desc_color || $blog_author_color || $blog_date_color){
	$output.='
	<style>
	'; if($blog_title_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .post-title{
		color:'.esc_attr($blog_title_color).' !important;
	}	
	'; } $output.='
	'; if($blog_desc_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .post-desc p{
		color:'.esc_attr($blog_desc_color).' !important;
	}
	'; } $output.='
	'; if($blog_author_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .author-name{
		color:'.esc_attr($blog_author_color).' !important;
	}
	'; } $output.='
	'; if($blog_date_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .post-date{
		color:'.esc_attr($blog_date_color).' !important;
	}
	'; } $output.='
	'; if($blog_category_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .post-taxonomies{
		color:'.esc_attr($blog_category_color).' !important;
	}	
	'; } $output.='
	'; if($btn_hover_text_color && $btn_hover_bg_color){ $output.='
	.business-blog-section-element.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
		border-color:'.esc_attr($btn_hover_bg_color).' !important;
	}
...
$get_image = wp_get_attachment_image_src( $expl_sec_img_id, 'sec_bg_img' );
	...
	$output='<section class="construction-projec-details-sec '.esc_attr($unique_css_class).'">
	...
	if($box_hover_bg){
		$output.='
		<style>
		.construction-projec-details-sec.'.wp_trim_words($unique_css_class).' .project-detail-carousel .item .overlay{
			background-color:'.esc_attr($box_hover_bg).' !important;
		}	
		</style>';
	}
	if($btn_hover_text_color && $btn_hover_bg_color){
		$output.='
		<style>
		.construction-projec-details-sec.'.wp_trim_words($unique_css_class).' .button:hover{
			color:'.esc_attr($btn_hover_text_color).' !important;
			background-color:'.esc_attr($btn_hover_bg_color).' !important;
			border-color:'.esc_attr($btn_hover_bg_color).' !important;
		}	
	...
	if($social_share_bg || $social_share_txt_color){
		$output.='
		<style>
		'; if($social_share_bg){ $output.='
		.construction-projec-details-sec.'.wp_trim_words($unique_css_class).' .social-share-sec{
			background-color:'.esc_attr($social_share_bg).' !important;
		}	
		'; } $output.='
		'; if($social_share_txt_color){ $output.='
		.construction-projec-details-sec.'.wp_trim_words($unique_css_class).' .social-share-sec ul li a i,.construction-projec-details-sec.'.wp_trim_words($unique_css_class).' .social-share-sec ul li a span{
			color:'.esc_attr($social_share_txt_color).' !important;
		}	
		'; } $output.='
		</style>';
	}	
						...
						<div class="row align-items-center">
							<div class="col-md-8 mb-5 mb-md-0 text-right">
								...
								<div class="deal-countdown" '; if($countdown_color){ $output.='style="color:'.esc_attr($countdown_color).'"'; } $output.='>
									<ul class="countdown-style-1 d-flex flex-wrap justify-content-end pl-0" data-countdown-to="'.esc_attr($countdown_date).'">
									</ul>
								</div>
								'; } $output.='
...
function ronby_fashion_woo_products_section_shortcode( $atts ) {
   ...
   function ronby_fashion_add_to_cart_button() {
	...
	if( $product->is_on_sale() ){ 
	$output.= "<span class='item-badge item-badge-red'>".esc_attr__('Sale', 'ronby')."</span>";
	}				
...
function ronby_fashion_button_shortcode( $atts ) {
	...
	$output='<div class="text-center" '; if($m_top){ $output.='style="margin-top:'.esc_attr($m_top).'"'; } $output.='>
			'; if($btn_label){ $output.='
	...
	$output='<section class="sale-campaign text-center background-cover" '; if($get_image[0]){ $output.='style="background-image:url('.esc_url($get_image[0]).')"'; } $output.='>
			<div class="overlay" '; if($overlay_bg || $padding_top || $padding_bottom){ $output.='style="'; if($overlay_bg){ $output.='background-color:'.esc_attr($overlay_bg).';'; } if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).';'; } $output.='"'; } $output.='>
				...
				'; if($btn2_label){ $output.='				
						<a href="'.esc_url($btn2_url).'" class="button rounded-capsule hover-background-primary" '; if($btn_text_color || $btn_bg_color){ $output.='style="'; if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.=';'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).''; } $output.='"'; } $output.='>
							'.esc_attr($btn2_label).'
						</a>
				'; } $output.='		
					</div>
...
function ronby_medical_service_box_sec_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 medical-service-box-sec">
				<div class="container">
					<div class="row">';
							$i=1;
							$c=0;
...
function ronby_construction_team_member_box_shortcode( $atts ) {
	...
	$output='<div class="team-item-1 construction-team-box '.esc_attr($unique_css_class).'">
						'; if($get_image[0]){ $output.='
							<div class="thumbnail animate-zoom">
								<a href="'.esc_url($link_to_url).'">
...
if($hover_icon_color || $hover_icon_bg){
	$output.='
	<style>
	.construction-team-box.'.wp_trim_words($unique_css_class).' .social-5 ul li:hover{
		color:'.esc_attr($hover_icon_color).' !important;
		background-color:'.esc_attr($hover_icon_bg).' !important;
	}	
	</style>';
...
function ronby_construction_testimonial_slider_shortcode( $atts ) {
	...
	$output='<section class="construction-slider '.esc_attr($unique_css_class).'" '; if($get_image[0]){ $output.='style="background:url('.esc_url($get_image[0]).');background-size:cover;"'; } $output.='>
...
if($slider_dot_bg){
	$output.='
	<style>
	.construction-slider.'.wp_trim_words($unique_css_class).' .owl-dot.active span{
		background-color:'.esc_attr($slider_dot_bg).' !important;
	}	
	.construction-slider.'.wp_trim_words($unique_css_class).'  .owl-dot.active{
		border-color:'.esc_attr($slider_dot_bg).' !important;		
	}	
	</style>';
}
...
function ronby_medical_cta_sec_shortcode( $atts ) {
	...
	$output='<section class="medical-cta section-emergency-care '.esc_attr($unique_css_class).'">
				<div class="overlay p-30-0-30" '; if($padding_top || $padding_bottom || $overlay_color){ $output.='style="'; if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).''; } $output.=';'; if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).''; } $output.=';'; if($overlay_color){ $output.='background-color:'.esc_attr($overlay_color).''; } $output.='"'; } $output.='>
				<div class="container">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.medical-cta.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.nearby-gyms.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fitness_call_to_action_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 fitness-cta '.esc_attr($unique_css_class).'" '; if($images[0] || $p_top || $p_bottom){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');background-position: top;'; } if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } $output.='"'; } $output.='>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.fitness-cta.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fitness_call_to_action_two_shortcode( $atts ) {
	...
	$output='<section class="fitness-cta-two '.esc_attr($unique_css_class).'" '; if($images[0]){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');background-size:cover;'; } $output.='"'; } $output.='>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.fitness-cta-two.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fitness_testimonial_slider_shortcode( $atts ) {
	...
	$heading_image_ids = explode(',',$heading_bgimage_glry);
	foreach( $heading_image_ids as $heading_image_id ){
    $heading_bgimage = wp_get_attachment_image_src( $heading_image_id, 'heading_bgimage_glry' );
	}
	
	$output='<section class="fitness-testimonial-slider '.esc_attr($unique_css_class).'" '; if($get_image[0]){ $output.='style="background:url('.esc_url($get_image[0]).');background-size:cover;"'; } $output.='>
	<div class="overlay p-30-0-30" '; if($padding_top || $padding_bottom || $overlay_color){ $output.='style="'; if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).''; } $output.=';'; if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).''; } $output.=';'; if($overlay_color){ $output.='background-color:'.esc_attr($overlay_color).''; } $output.='"'; } $output.='>
			<div class="container">
			'; if($headline1 || $headline2){ $output.='
			<div class="section-header-style-6 p-30-0-30  text-center"<div class="section-header-style-6"'; if($heading_bgimage[0]){ $output.=' style="background: url('.esc_url($heading_bgimage[0]).') center bottom no-repeat;"'; } $output.='>>
			'; if($headline1){ $output.='
			<h2 class="section-title" '; if($headline1_color || $headline1_padding_left){ $output.='style="'; if($headline1_color){ $output.='color:'.esc_attr($headline1_color).';'; } if($headline1_padding_left){ $output.='padding-left:'.esc_attr($headline1_padding_left).''; } $output.='"'; } $output.='>'.esc_attr($headline1).'</h2>
			'; } $output.='
			'; if($headline2){ $output.='
...
if($slider_dot_bg){
	$output.='
	<style>
	.fitness-testimonial-slider.'.wp_trim_words($unique_css_class).' .owl-dot.active span{
		background-color:'.esc_attr($slider_dot_bg).' !important;
	}	
	.fitness-testimonial-slider.'.wp_trim_words($unique_css_class).'  .owl-dot.active{
		border-color:'.esc_attr($slider_dot_bg).' !important;		
	}	
	</style>';
}
...
function ronby_medical_appointment_sec_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 m-appointment-sec '.esc_attr($unique_css_class).'" '; if($images[0] || $p_top || $p_bottom){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');background-size: cover;'; } if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } $output.=';
	"'; } $output.='>
	<div class="row">
	...
	<button type="submit" class="button rounded-capsule button-primary" '; if($btn_text_color || $btn_bg_color){ $output.='style="'; if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.=';'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).''; } $output.='"'; } $output.='>'.esc_attr($btn_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="ap-contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader').'"/></button>
	</div>	
	<div class="col-lg-12">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.m-appointment-sec.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_medical_appoint_form_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 medical-appointment-form-two '.esc_attr($unique_css_class).'">
				<div class="container">
					<div class="row align-items-center">
						<div class="col-lg-12 color-inverse">
							...
							<div class="form-style-6">
								...
								$output .= '</select>
									...
									<div class="form-group">
										<textarea class="input-styled" name="message" id="m-apf-sendermessage" rows="9" placeholder="'.esc_attr($message_field_placeholder).'" required></textarea>
									</div>
									<div class="form-group mt-4">

									<button class="button button-secondary rounded" type="submit" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="m-apf-contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader').'"/></button>										
									</div>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.medical-appointment-form-two.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
		border-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
...
function ronby_medical_image_gallery_shortcode( $atts ) {
	...
	$output='<section class="medical-image-gallery p-30-0-30" '; if($p_top || $p_bottom){ $output.='style="'; if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } $output.='"'; } $output.='>
...
function ronby_medical_team_details_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30 medical-team-details-sec">
			<div class="container">
				<div class="row justify-content-center">
					<div class="col-xl-10">
...
function ronby_medical_doctor_profile_shortcode( $atts ) {
	...
	$output='<section class="doctor-profile-sec">
				<div class="team-item-3">
					<div class="thumbnail animate-zoom">
					'; if($images[0]){ $output.='
						<a href="'.esc_url($link_to_url).'">
					...
					<div class="info-box" '; if($bg_color || $p_top || $p_bottom || $p_left || $p_right){ $output.='style="'; if($bg_color){ $output.='background-color:'.esc_attr($bg_color).';'; } if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } if($p_left){ $output.='padding-left:'.esc_attr($p_left).';'; } if($p_right){ $output.='padding-right:'.esc_attr($p_right).';'; }  $output.='"'; } $output.='>
					'; if($doctor_name){ $output.='
					<a href="'.esc_url($link_to_url).'" class="no-color">
						<h3 class="item-title hover-color-primary animate-300 '; if($bg_color){ $output.='mt-0'; } $output.='" '; if($doctor_name_color){ $output.='style="color:'.esc_attr($doctor_name_color).'"'; } $output.='><span class="text-highlighted" '; if($doctor_title_color){ $output.='style="color:'.esc_attr($doctor_title_color).'"'; } $output.='>'.esc_attr($doctor_title).'</span> '.esc_attr($doctor_name).'</h3>
					</a>
					'; } $output.='
	...
	$output='<section class="p-30-0-30">
			<div class="form-style-2">
						...
						$output .='</div></div>';
							...
							<button class="button button-primary rounded" type="submit" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'<img src="'.esc_url(plugin_dir_url( __FILE__ ) .'images/msg_loader.gif').'" class="mdf-contact-form-loader display-none" style="width:20px;height:20px;" alt="'.esc_attr__('loader').'"/></button>								
							</div>
							<div class="col-lg-12 nopadding">
...
function ronby_construction_single_image_shortcode( $atts ) {
	...
	$output='<section class="p-30-0-30">
				<div class="image-box-1">
				'; if($images[0]){ $output.='
					<img src="'.esc_url($images[0]).'" alt="'.esc_attr__('featured-image','ronby').'" '; if($img_width){ $output.='style="width:'.esc_attr($img_width).'"'; } $output.='/>
				'; } $output.='	
					<div class="behind-bg" '; if($behind_img_bg){ $output.='style="background-color:'.esc_attr($behind_img_bg).'"'; } $output.='></div>
				</div>
			</section>';
...
function ronby_construction_team_details_shortcode( $atts ) {
	...
	$output='<div class="team-detail-1 '.esc_attr($unique_css_class).'">
							<div class="row no-gutters">
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.team-detail-1.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
function ronby_fashion_header_section_shortcode( $atts ) {
	...
	$output='<section class="policies fashion-header-section '.esc_attr($unique_css_class).'">
	<div class="bg_img" '; if($images[0] || $part1_height){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');background-position:center;'; } if($part1_height){ $output.='height:'.esc_attr($part1_height).';'; } $output.='"'; } $output.='>
	...
	<div class="container">
	<div class="row">
	<div class="col-md-12 color-white pt-224 text-center sale-campaign">
	'; if($f_heading1){ $output.='
	<h1'; if($f_heading1_color){ $output.=' style="color:'.esc_attr($f_heading1_color).'"'; } $output.='>'.esc_attr($f_heading1).'</h1>
	'; } $output.='
	'; if($f_heading2){ $output.='
	<h4'; if($f_heading2_color){ $output.=' style="color:'.esc_attr($f_heading2_color).'"'; } $output.='>'.$f_heading2.'</h4>
	'; } $output.='
	'; if($f_heading3){ $output.='
	<h6 class="mb-4" '; if($f_heading3_color){ $output.='style="color:'.esc_attr($f_heading3_color).'"'; } $output.='>'.esc_attr($f_heading3).'</h6>
	'; } $output.='
	'; if($btn_label){ $output.='
	<a class="button rounded-capsule hover-background-primary" href="'.esc_url($btn_url).'" '; if($btn_text_color || $btn_bg_color){ $output.='style="'; if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.=';'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).''; } $output.='"'; } $output.='>'.esc_attr($btn_label).'</a>
	'; } $output.='
	</div>
	</div>
	</div>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.fashion-header-section.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
}	
if($btn_padding_top && $btn_padding_bottom && $btn_padding_left && $btn_padding_right){
	$output.='
	<style>
	.fashion-header-section.'.wp_trim_words($unique_css_class).' .button{
		padding:'.esc_attr($btn_padding_top).' '.esc_attr($btn_padding_right).' '.esc_attr($btn_padding_bottom).' '.esc_attr($btn_padding_left).' !important;
	}	
	</style>';
}
if($btn_font_size){
	...
	<style>
	.fashion-header-section.'.wp_trim_words($unique_css_class).' .button{
		font-size:'.esc_attr($btn_font_size).'  !important;
	}	
	</style>';
}
...
/*****************************
Business Projects Grid-2
******************************/
//Function for Business Projects Grid-2
function ronby_business_projects_grid_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'number_of_items' => '',
	'title1_color' => '',
	'title2_color' => '',
	...
	if(!empty($features)){
      $out = '<div id="filters" class="filter-nav-1" '; if($categories_sec_bg){$out.='style="background-color:'.esc_attr($categories_sec_bg).'"'; }$out.='>	  
					<div class="row">					
					<div class="col-lg-3">
					'; if($heading){$out.='
						<div class="lb" '; if($heading_c){$out.='style="color:'.esc_attr($heading_c).'"'; }$out.='>'.esc_attr($heading).'</div>
					'; }$out.='		
					</div>					
				<div class="col-lg-9">	
				  <ul class="no-style text-right">
				  '; if($showall){$out.='
					<li  class="hover-color-primary active-color-primary after-background-primary animate-300 is-checked active" data-filter="*" '; if($categories_c){$out.='style="color:'.esc_attr($categories_c).'"'; }$out.='>
						<span class="text">'.esc_attr($showall).'</span>
				  </li>';
				  }
      $features = !empty($features) ? explode("\n", trim($features)) : array(); 
	  ...
	  $feature = strip_tags($feature);
	  $c++;
      $out .= '<li class="hover-color-primary active-color-primary after-background-primary animate-300" data-filter=".'.$feature.'" '; if($categories_c){$out.='style="color:'.esc_attr($categories_c).'"'; }$out.='>';
				
	  $out .=''.htmlspecialchars_decode($feature).'</li>';
      }
      $out .= '</ul>
				  </div>
				  </div>
	...
	$output='<section class="p-30-0-30 business-projects-grid-2">
	<div class="container">';	
	$output .= ''.do_shortcode($content).'';
	$output .= '</div><div class="">';
	$output .='<div class="masonry grid masonry-no-gutter masonry-5-columns">
				<div class="masonry-sizer"></div>
				<div class="masonry-gutter"></div>
					';
					$i=1;
					...
					$link_to_url =$b['link_to_url'.$c.''];
					$item_features =$b['item_features'.$c.''];
					$large_width_img =$b['large_width_img'.$c.''];
					$output .='				
				<div class="masonry-item element-item '.esc_attr( $item_features ).'  '; if($large_width_img == "yes"){ $output.='masonry-item-width-2x'; } $output.='" data-category="'.esc_attr( $item_features ).'">
					<article class="article-with-overlay-3">
					'; if($img_url){ $output.='
						<div class="thumbnail">
							<img src="'.esc_url($img_url).'" alt="'.esc_attr__('featured-image','ronby').'"/>
...
add_shortcode('ronby_shortcode_for_business_projects_grid_two', 'ronby_business_projects_grid_two_shortcode');

/*****************************
Fitness Testimonial Slider
******************************/
//Function for Fitness Testimonial Slider
function ronby_fitness_testimonial_slider_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'bg_color' => '',
	'text_color' => '',
	'name_color' => '',
	...
	$image_ids = explode(',',$border_left_img);
	foreach( $image_ids as $image_id ){
    $border_left_img = wp_get_attachment_image_src( $image_id, 'border_left_img' );
	}
	...
	$unique_css_class=$shuffled;		
	$output='<section class="p-30-0-30 fitness-testimonial-slider-2 '.esc_attr($unique_css_class).'" '; if($bg_color){ $output.='style="background-color:'.esc_attr($bg_color).'"'; } $output.='>
			<div class="container">
			<div class="testimonial-slider-5 owl-carousel">';
	$i=1;
	$c=0;
	while ($i<=$number_of_testimonial){
	$c++;
	...
	$output.='<div class="item">
						<div class="row">
							...
							<div class="col-sm-8" '; if($border_left_img[0]){ $output.='style="background:url('.esc_url($border_left_img[0]).') left top no-repeat;"'; } else { $output.='style="background:url('.esc_url(plugin_dir_url(__FILE__)).'images/testimonial5-border.png) left top no-repeat;"'; } $output.='>
							'; if($text){ $output.='
								<div class="testimonial-quote mb-2" '; if($text_color){ $output.='style="color:'.esc_attr($text_color).'"'; } $output.='>
								'.$text.'	
								</div>
							'; } $output.='	
								<div class="testimonial-author ">
	...
	$output.='
	<style>
	.fitness-testimonial-slider-2.'.wp_trim_words($unique_css_class).' .owl-stage-outer{
		padding-top:50px;		
	}
	.fitness-testimonial-slider-2.'.wp_trim_words($unique_css_class).' .avatar{
		margin-top: -50px;		
	}	
	</style>';
	}
...
add_shortcode('ronby_shortcode_for_fitness_testimonial_slider_two', 'ronby_fitness_testimonial_slider_two_shortcode');


/*****************************
Fitness Photo Gallery Grid
******************************/
//Function for Fitness Photo Gallery Grid
function ronby_fitness_photo_gallery_grid_shortcode( $atts ) {
	extract( shortcode_atts( array(
    'showall' => '',
	'features' => '',
	'number_of_items' => '',
	...
	if(!empty($features)){
      $output = '<div class="tabs-filter tabs-filter-style-1 text-right">
						<div class="filter-nav-2 mb-2">	  
							<div id="filters">
				  <ul class="list-unstyled">
				  '; if($showall){ $output.='
					<li  class="tab active-color-primary hover-color-primary animate-300 is-checked active" data-filter="*">
	...
	$out = '<section class="fitness-photo-gallery-grid '.esc_attr($unique_css_class).' p-30-0-30"><div class="container">';
    $out .= ''.do_shortcode($content).'';
	$out .= '</div><div class="container nopadding"><div class="tabs-filter tabs-filter-default d-md-flex"><div class="flex-fill">
			<div class="grid">';
							$i=1;
							$c=0;
							while ($i <= $number_of_items){
							...
							$item_img =$b['item_img'.$c.''];
							$item_features =$b['item_features'.$c.''];
$out .='<div class="col-sm-6 col-md-4 col-lg-4 element-item '.esc_attr( $item_features ).' pl-05 pr-05" data-category="'.esc_attr( $item_features ).'">
		<div class="article-with-overlay-4">
		'; if($item_img){$out.='
			<a href="'.esc_url($item_img).'" class="thumbnail">
				<img src="'.esc_url($item_img).'" alt="'.esc_attr__('featured-image','ronby').'">
			</a>
		'; }$out.='	
			<a href="'.esc_url($item_img).'" data-lightbox="featured-image">
				<div class="overlay overlay-fill d-flex align-items-center flex-column justify-content-center">
					<div class="icon">
						<i class="fas fa-search"></i>
					</div>													
...
if($active_tab_text_color && $active_tab_bg_color){
	$out.='
	<style>
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .active-color-primary.active{
		color:'.esc_attr($active_tab_text_color).' !important;
	}	
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .active-color-primary.active{
		background-color:'.esc_attr($active_tab_bg_color).' !important;
		border-color:'.esc_attr($active_tab_bg_color).' !important;
	}	
	</style>';
} 
if($tab_border_color){
	$out.='
	<style>
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .tab{
		border-color:'.esc_attr($tab_border_color).' !important;
	}	
	</style>';
}
if($box_hover_color){
	$out.='
	<style>
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .article-with-overlay-4:hover .overlay{
		background-color:'.esc_attr($box_hover_color).' !important;
	}		
	</style>';
}
...
if($hover_icon_color || $hover_icon_bg || $hover_text_color){
	...
	<style>
	'; if($hover_icon_color || $hover_icon_bg){$out.='
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .icon{
		color:'.esc_attr($hover_icon_color).' !important;
		background-color:'.esc_attr($hover_icon_bg).' !important;
	}
	'; } if($hover_text_color){ $out.='
	.fitness-photo-gallery-grid.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .post-title{
		color:'.esc_attr($hover_text_color).' !important;
	}
	'; }$out.='
	</style>';
...
add_shortcode('ronby_shortcode_for_fitness_photo_gallery_grid', 'ronby_fitness_photo_gallery_grid_shortcode');

/*****************************
Fitness Call To Action- 3
******************************/
//Function for Fitness Call To Action- 3
function ronby_fitness_call_to_action_three_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading2' => '',
	'description' => '',
	...
	$output='<section class="fitness-cta-two '.esc_attr($unique_css_class).'" '; if($images[0]){ $output.='style="'; if($images[0]){ $output.='background-image:url('.esc_url($images[0]).');background-size:cover;'; } $output.='"'; } $output.='>
			...
			'; if($heading1){ $output.='
			<h1 class="fitness-cta-heading1 mb-4" '; if($headline1_color){ $output.='style="color:'.esc_attr($headline1_color).'"'; } $output.='>'.esc_attr($heading1).'</h1>	
			'; } $output.='
			'; if($heading2){ $output.='
			<h2 class="fitness-cta-heading2 mb-4" '; if($headline2_color){ $output.='style="color:'.esc_attr($headline2_color).'"'; } $output.='>'.$heading2.'</h2>
			'; } $output.='
			'; if($description){ $output.='
			<div class="fitness-cta-p mb-4" '; if($description_color){ $output.='style="color:'.esc_attr($description_color).'"'; } $output.='>'.$description.'</div>
			'; } $output.='
			'; if($button_label){ $output.='
			<a class="fitness-cta-button button rounded-capsule button-primary mt-4" href="'.esc_url($button_url).'" target="_self" '; if($btn_text_color || $btn_bg_color){ $output.='style="'; if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.=';'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).''; } $output.='"'; } $output.='>'.esc_attr($button_label).'</a>
			'; } $output.='				
...
add_shortcode('ronby_shortcode_for_fitness_call_to_action_three', 'ronby_fitness_call_to_action_three_shortcode');

/*****************************
Fitness Program Section-2
******************************/
//Function for Fitness Program Section-2
function ronby_fitness_program_section_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading' => '',
	'heading_dif_colored' => '',
	'desc' => '',
	...
	$output='<section class="section-join-us '.esc_attr($unique_css_class).' '; if($border_animation=="yes"){ $output.=' bdron'; } $output.='" '; if($bg_color){ $output.='style="background-color:'.esc_attr($bg_color).'"'; } $output.='>
				<div class="container" id="un343">
					<div class="row">
					<div class="col-md-6">
					<div class="section-content pl-0 pr-0 bg-none" '; if($padding_top || $padding_bottom){ $output.='style="'; if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).';'; } $output.='"'; } $output.='>
						<div class="section-header-style-7">
						'; if($heading){ $output.='
							<h2 class="section-title" '; if($title_color){ $output.='style="color:'.esc_attr($title_color).'"'; } $output.='>'.wp_kses(__($ret),$allowed_html_array).'</h2>
						'; } $output.='
						...
						<div class="bordered-top-uls"></div>
						'; } $output.='
						'; if(($feature1_icon && $feature1_title) || ($feature2_icon && $feature2_title) || ($feature3_icon && $feature3_title) ){ $output.='
						<ul class="section-list-item d-flex flex-wrap align-items-center">
						'; if($feature1_icon && $feature1_title){ $output.='
							<li>
								<div class="item-icon color-primary">
				...
				<div class="image-box-1">
				'; if($images[0]){ $output.='
					<img src="'.esc_url($images[0]).'" alt="'.esc_attr__('featured-image','ronby').'" '; if($img_width || $img_top){ $output.='style="'; if($img_width){ $output.='width:'.esc_attr($img_width).';'; } if($img_top){ $output.='top:'.esc_attr($img_top).''; } $output.='"'; } $output.='/>
				'; } $output.='					
				</div>
				</div>
				</div>
...
add_shortcode('ronby_shortcode_for_ronby_fitness_program_section_two', 'ronby_fitness_program_section_two_shortcode');

/*****************************
Restaurant Photo Gallery
******************************/
//Function for Restaurant Photo Gallery
function ronby_restaurant_photo_gallery_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'image' => '',
	'hover_icon_color' => '',
	'hover_icon_bg' => '',
	...
	$output='<section class="p-30-0-30 restaurant-photo-gallery '.esc_attr($unique_css_class).'">
			<div class="masonry masonry-no-gutter masonry-3-columns">
				<div class="masonry-sizer"></div>
				<div class="masonry-gutter"></div>
					';
	...
	$output .='				
				<div class="masonry-item">
					...
					'; if($images[$c]){ $output.='
						<div class="thumbnail">
							<img src="'.esc_url($images[$c]).'" alt="'.esc_attr__('featured-image','ronby').'"/>
						</div>
					'; } $output.='	
						<div class="overlay overlay-fill d-flex flex-column justify-content-end">			
							<div class="item-buttons align-self-center">
								<a href="'.esc_url($images[$c]).'" class="quick-view background-primary color-inverse icon" data-lightbox="featured-image">
									<i class="fas fa-plus"></i>
								</a>								
							</div>
						</div>				
...
if($box_hover_color){
	$output.='
	<style>
	.restaurant-photo-gallery.'.wp_trim_words($unique_css_class).' .article-with-overlay-3:hover .overlay{
		background-color:'.esc_attr($box_hover_color).' !important;
	}		
	</style>';
}
...
if($hover_icon_color || $hover_icon_bg){
	$output.='
	<style>
	'; if($hover_icon_color || $hover_icon_bg){ $output.='
	.restaurant-photo-gallery.'.wp_trim_words($unique_css_class).' .article-with-overlay-3 .icon{
		color:'.esc_attr($hover_icon_color).' !important;
		background-color:'.esc_attr($hover_icon_bg).' !important;
	}
	'; } $output.='
...
add_shortcode('ronby_shortcode_for_restaurant_photo_gallery', 'ronby_restaurant_photo_gallery_shortcode');

/*****************************
Restaurant About us section
******************************/
//Function for Restaurant About us Section
function ronby_restaurant_about_us_sec_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading1_color' => '',
	'heading2' => '',
	...
	$output='<section class="restaurant_about_us_section p-30-0-30">
			<div class="container">
				<div class="row">
				<div class="col-md-6">
				<div class="section-header-style-15 mb-3">
				'; if($heading1){ $output.='
					<h4 class="section-header-sub-title color-primary" '; if($heading1_color){ $output.='style="color:'.esc_attr($heading1_color).'"'; } $output.='>'.esc_attr($heading1).'</h4>
				'; } $output.='	
				'; if($heading2){ $output.='
				...
				'; if($desc){ $output.='
				<div class="text-center" '; if($desc_color){ $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>
				<p>'.$desc.'</p>
				</div>
				'; } $output.='
				<div class="chef mb-30 mt-4">
				<div class="row m-auto">
				'; if($chef_img){ $output.='
				<div class="col-md-2 offset-md-3">
				<div class="avatar" style="background-image: url('.esc_url($chef_img).')"></div>
				</div>
				'; } $output.='
				<div class="col-md-6">
				<div class="align-self-center mt-2">
				'; if($chef_name){ $output.='
				<h5 class="ex-bold mb-15" '; if($chef_name_color){ $output.='style="color:'.esc_attr($chef_name_color).'"'; } $output.='>'.esc_attr($chef_name).'</h5> 
				'; } $output.='
				'; if($chef_signature){ $output.='
				<img src="'.esc_url($chef_signature).'" alt="'.esc_attr__('signature','ronby').'" class="signature">
				'; } $output.='
				</div>
				</div>
				</div>
				...
				<div class="col-md-6">
				'; if($right_side_image){ $output.='
				<img src="'.esc_url($right_side_image).'" alt="'.esc_attr__('featured-image','ronby').'">
				'; } $output.='				
				</div>				
				</div>
			</div>	
...
add_shortcode('ronby_shortcode_for_restaurant_about_us_section', 'ronby_restaurant_about_us_sec_shortcode');

/*****************************
Restaurant Button
******************************/
//Function for Restaurant Button
function ronby_restaurant_button_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'btn_label' => '',
	'btn_url' => '',
	'btn_bg_color' => '',
	...
	$output='<div class="text-center button-wrapper '.esc_attr($unique_css_class).'">
			'; if($btn_label){ $output.='
				<a href="'.esc_url($btn_url).'" class="button  comment-submit button-primary" '; if($btn_bg_color || $btn_text_color){ $output.='style="'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).';border-color: '.esc_attr($btn_bg_color).';'; } if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.='"'; } $output.='>'.esc_attr($btn_label).'</a>
			'; } $output.='	
			</div>';
...
add_shortcode('ronby_shortcode_for_restaurant_button', 'ronby_restaurant_button_shortcode');

/****************************
Medical Accordion
*****************************/
//This function is for Medical Accordion
function ronby_accordion_one_shortcode( $atts ) {
    extract(shortcode_atts(array(
    'accordion_head_color'        => '',
    'accordion_desc_bg_color'     => '',
	'accordion_head_text_color'   => '',
	'accordion_desc_text_color'   => '',	
	'headline' => '',
	'desc' => '',
	'headline_color' => '',
	'desc_color' => '',	
   ...
   $output ='<section class="p-30-0-30">
   	<div class="section-header-style-10">
	'; if($headline){ $output.='
	<h4 class="section-sub-title" '; if($headline_color){ $output.='style="color:'.esc_attr($headline_color).'"'; } $output.='>'.esc_attr($headline).'</h4>
	'; } $output.='
	'; if($desc){ $output.='
	<h2 class="section-title" '; if($desc_color){ $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>'.$desc.'</h2>
	'; } $output.='
	</div>
	<!-- Accordion begin -->
    ...
    $desc  =$b['desc'.$c.''];
	 ...
	 $output .='  
			  <!-- Section 1 -->
			  <div class="accordion_in style2 ';
	 $output .='" '; if($accordion_desc_bg_color || $border_color){ $output.='style="'; if($accordion_desc_bg_color){ $output.='background-color:'.esc_attr($accordion_desc_bg_color).';'; } if($border_color){ $output.='border:2px solid '.esc_attr($border_color).';'; } $output.='"'; } $output.='>
                <div class="acc_head mb-0 border-none" '; if($accordion_head_color || $accordion_head_text_color){ $output.='style="'; if($accordion_head_color){ $output.='background-color:'.esc_attr($accordion_head_color).';'; } if($accordion_head_text_color){ $output.='color:'.esc_attr($accordion_head_text_color).';'; } $output.='"'; } $output.='>'.esc_attr($title).'</div>
                <div class="acc_content no-lpadd" '; if($accordion_desc_bg_color || $accordion_desc_text_color){ $output.='style="'; if($accordion_desc_bg_color){ $output.='background-color:'.esc_attr($accordion_desc_bg_color).';'; } if($accordion_desc_text_color){ $output.='color:'.esc_attr($accordion_desc_text_color).';'; } $output.='"'; } $output.='>
                  <p>'.wp_kses(__($desc), $allowed_html_array).'</p>
                </div>
              </div>             
	';
	$i++;
...
add_shortcode('ronby_shortcodes_for_accordion_one', 'ronby_accordion_one_shortcode');

/*****************************
Medical Newsletter Section
******************************/
//Function for Medical Newsletter Section
function ronby_medical_newsletter_sec_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'headline' => '',
	'desc' => '',
	'headline_color' => '',
	...
	$output='<section class="newsletter medical-newsletter p-30-0-30">
			<div class="container-fluid">
				<div class="row">
					...
					<div class="col-md-4">
						<form id="fashion-newsletter" method="POST">
							...
							<input type="email" class="input-styled text-left" placeholder="Enter your Email" id="newsletter-email" required>
							
							<div class="pt-4">
								<button type="submit" class="button button-primary rounded" '; if($btn_text_color || $btn_bg_color){ $output.='style="'; if($btn_text_color){ $output.='color:'.esc_attr($btn_text_color).''; } $output.=';'; if($btn_bg_color){ $output.='background-color:'.esc_attr($btn_bg_color).''; } $output.='"'; } $output.='><span style="display:inline-block">'.esc_attr($btn_label).'</span> <span style="display:inline-block"><img src="'.plugin_dir_url(__FILE__).'/images/newsletter_loader.gif" alt="newsletter-loader" style="width:30px;" class="newsletter-loader"/></span>	</button>
								<div class="newsletter-result"></div>
							</div>
						</form>						
					</div>
...
add_shortcode('ronby_shortcode_for_medical_newsletter_sec', 'ronby_medical_newsletter_sec_shortcode');

/*****************************
Medical Feature Box
******************************/
//Function for Medical Feature Box
function ronby_medical_feature_box_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading' => '',
	'heading_color' => '',
	'desc' => '',
	...
	$output='<section class="medical_feature_box" '; if($p_top || $p_bottom){ $output.='style="'; if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } $output.='"'; } $output.='>
			<div class="article-with-icon-3 text-center mb-0">
				'; if($icon){ $output.='
				<div class="icon color-primary">
					<i class="'.esc_attr($icon).'" '; if($icon_color){ $output.='style="color:'.esc_attr($icon_color).'"'; } $output.='></i>
				</div>
...
add_shortcode('ronby_shortcode_for_medical_feature_box', 'ronby_medical_feature_box_shortcode');

/*****************************
Medical Service Tab Grid 
******************************/
//Function for Medical Service Tab Grid 
function ronby_medical_service_tab_grid_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'number_of_items' => '',
	'title_color' => '',
	'desc_color' => '',
	...
	if(!empty($features)){
      $out = '<div class="container mb-4"><div class="row"><div class="col-md-12"><div class="flex-auto">
						<div class="filter-nav-5">	  
							<div id="filters">
				  <ul class="list-unstyled pl-0">
				  '; if($showall){$out.='
					<li  class="tab active-color-primary hover-color-primary animate-300 cursor-point active" data-filter="*">
						<span class="text">'.esc_attr($showall).'</span>
				  </li>';
				  }
      $features = !empty($features) ? explode("\n", trim($features)) : array(); 
	  ...
	  $feature = strip_tags($feature);
	  $c++;
      $out .= '<li class="tab active-color-primary hover-color-primary animate-300 cursor-point" data-filter=".'.$feature.'">';
	...
	$output='<section class="p-30-0-30 medical-service-tab-grid '.esc_attr($unique_css_class).'">
	';$output .= ''.do_shortcode($content).'';
	  $output .= '
				<div class="flex-fill">
					<div class="grid">';
...
if(($counter == 1) || ($counter == 2)){								
	$output.='<div class="col-lg-6 element-item '.esc_attr( $item_features ).'" data-category="'.esc_attr( $item_features ).'">
							<div class="service-item-2 thumb-left-style">
								<div class="row no-gutters align-items-center">
	...
	$output.='	 </div>
	  ...
	  $output.='<script>
	  jQuery(function() {       
	  jQuery(".filter-nav-5 li").click(function() {   
			jQuery(".filter-nav-5 li").removeClass("active");      
			jQuery(this).addClass("active");      
		  });
		});
	  </script>';
	  
if($active_tab_color && $active_tab_bg_color){
	$output.='
	<style>
	.medical-service-tab-grid.'.wp_trim_words($unique_css_class).' .active-color-primary.active{
		color:'.esc_attr($active_tab_color).' !important;
		background-color:'.esc_attr($active_tab_bg_color).' !important;
	}		
	</style>';
...
add_shortcode('ronby_shortcode_for_medical_service_tab_grid', 'ronby_medical_service_tab_grid_shortcode');

/*****************************
Construction Feature Box-3
******************************/
//Function for Construction Feature Box-3
function ronby_construction_feature_box_three_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading' => '',
	'heading_color' => '',
	'desc' => '',
	...
	$output='<section class="construction_feature_box_three p-30-0-30 pl-4 pr-4 mt-4 mb-4 '.esc_attr($unique_css_class).'" '; if($p_top || $p_bottom){ $output.='style="'; if($p_top){ $output.='padding-top:'.esc_attr($p_top).';'; } if($p_bottom){ $output.='padding-bottom:'.esc_attr($p_bottom).';'; } $output.='"'; } $output.='>
			<div class="article-with-icon-3 text-center mb-0">
				'; if($icon){ $output.='
				<div class="icon color-primary">
					<i class="'.esc_attr($icon).'" '; if($icon_color){ $output.='style="color:'.esc_attr($icon_color).'"'; } $output.='></i>
				...
				'; } $output.='
				'; if($link_to_url){ $output.='
				<div class="feature_item_url">
				<a href="'.esc_url($link_to_url).'" class="arrow" '; if($icon_bg_color){ $output.='style="background-color:'.esc_attr($icon_bg_color).';"'; } $output.='>
					<i class="fas fa-angle-right"></i>
				</a>
				</div>
				'; } $output.='
...
if($box_hover_color){
	$output.='
	<style>
	.construction_feature_box_three.'.wp_trim_words($unique_css_class).':hover {
		background-color:'.esc_attr($box_hover_color).' !important;
		transition:0.5s;
	}		
	</style>';
...
if($hover_icon_color){
	$output.='
	<style>
	.construction_feature_box_three.'.wp_trim_words($unique_css_class).':hover .icon i{
		color:'.esc_attr($hover_icon_color).' !important;
	}		
	</style>';
}
if($hover_title_color){
	$output.='
	<style>
	.construction_feature_box_three.'.wp_trim_words($unique_css_class).':hover .item-title{
		color:'.esc_attr($hover_title_color).' !important;
	}		
	</style>';
}	
if($hover_desc_color){
	$output.='
	<style>
	.construction_feature_box_three.'.wp_trim_words($unique_css_class).':hover .item-text{
		color:'.esc_attr($hover_desc_color).' !important;
	}		
	</style>';
}
if($hover_icon_bg_color){
	$output.='
	<style>
	.construction_feature_box_three.'.wp_trim_words($unique_css_class).':hover .arrow{
		background-color:'.esc_attr($hover_icon_bg_color).' !important;
		color:'.esc_attr($box_hover_color).' !important;
	}		
	</style>';
}
...
add_shortcode('ronby_shortcode_for_construction_feature_box_three', 'ronby_construction_feature_box_three_shortcode');

/*****************************
Fitness Trainer Profile Box
******************************/
//Function for Fitness Trainer Profile Box
function ronby_fitness_trainer_profile_box_shortcode( $atts ) {
	extract( shortcode_atts( array(
		'trainer_name' => '',
		'trainer_designation' => '',
		'trainer_description' => '',
	...
	$output='<section class="p-30-0-30 fitness-section-teams">
			<article class="team-item-2" '; if($box_margin_top || $box_margin_bottom){ $output.='style="'; if($box_margin_top){ $output.='margin-top:'.esc_attr($box_margin_top).';'; } if($box_margin_bottom){ $output.='margin-bottom:'.esc_attr($box_margin_bottom).''; } $output.='"'; } $output.='>
			<div '; if($border_c || $box_bg_color){ $output.='style="'; if($border_c){ $output.='border:1px solid '.esc_attr($border_c).';'; } if($box_bg_color){ $output.='background-color: '.esc_attr($box_bg_color).''; } $output.='"'; } $output.='>
			'; if($trainer_img_url){ $output.='
			...
			<div class="shadow-brdr rnd-pd text-left pl-4">
			'; if($trainer_name){ $output.='
			<a href="'.esc_url($trainer_link_url).'">
				<h3 class="member-name" '; if($trainer_name_color){ $output.='style="color:'.esc_attr($trainer_name_color).'"'; } $output.='>
					'.esc_attr($trainer_name).'
			...
			'; } $output.='
			'; if($trainer_description){ $output.='
			<div class="member-role color-primary mb-2" '; if($trainer_description_color){ $output.='style="color:'.esc_attr($trainer_description_color).'"'; } $output.='>
				'.$trainer_description.'
			</div>
			'; } $output.='			
			'; if($trainer_phone){ $output.='
			<div class="member-role color-primary text-left mb-2" '; if($trainer_phone_color || $icon_p_left){ $output.='style="'; if($trainer_phone_color){ $output.='color:'.esc_attr($trainer_phone_color).';'; } if($icon_p_left){ $output.='padding-left:'.esc_attr($icon_p_left).' !important'; } $output.='"'; } $output.='>
				<i class="fas fa-phone-volume mr-2" '; if($trainer_icon_color){ $output.='style="color:'.esc_attr($trainer_icon_color).'"'; } $output.='></i> '.esc_attr($trainer_phone).'
			</div>
			'; } $output.='	
			'; if($trainer_email){ $output.='
			<div class="member-role color-primary text-left mb-4" '; if($trainer_email_color || $icon_p_left){ $output.='style="'; if($trainer_email_color){ $output.='color:'.esc_attr($trainer_email_color).';'; } if($icon_p_left){ $output.='padding-left:'.esc_attr($icon_p_left).' !important'; } $output.='"'; } $output.='>
				<i class="far fa-envelope mr-2" '; if($trainer_icon_color){ $output.='style="color:'.esc_attr($trainer_icon_color).'"'; } $output.='></i> '.esc_attr($trainer_email).'
			</div>
			'; } $output.='
			'; if($button_label){ $output.='
			<div class="member-role color-primary mb-4">
				<a href="'.esc_url($trainer_link_url).'" class="no-color outline-button">'.esc_attr($button_label).'</a>
			</div>
			'; } $output.='			
			</div>
		</div>	
...
add_shortcode('ronby_shortcode_for_trainer_profile_box', 'ronby_fitness_trainer_profile_box_shortcode');

/******************* 
Construction Pricing Table 
********************/
//This function is for Construction Pricing Table 

function ronby_construction_pricing_table_shortcode( $atts ) {
extract( shortcode_atts( array(
	'plan_name'		 => '',
	'plan_currency'  => '',
	'plan_price'     => '',
...
$output='<section class="p-30-0-30 construction_pricing_table '.esc_attr($unique_css_class).'">
<div class="pricing-box-home2 text-center pb-4" '; if($border_color){ $output.='style="border:1px solid '.esc_attr($border_color).';"'; } $output.='>
   '; if($plan_name){ $output.='
	<h2 class="font-bold font25 font-white uppercase mb-0" '; if($plan_name_color || $heading_bg){ $output.='style="'; if($plan_name_color){ $output.='color:'.esc_attr($plan_name_color).' !important;'; } if($heading_bg){ $output.='background-color:'.esc_attr($heading_bg).''; } $output.='"'; } $output.='>'.esc_attr($plan_name).'</h2>
	'; } $output.='
   <div class="price font-thin-xs font45 font-white" '; if($price_bg){ $output.='style="'; if($price_bg){ $output.='background-color:'.esc_attr($price_bg).''; } $output.='"'; } $output.='>
   '; if($plan_price){ $output.='
	<span '; if($plan_price_color){ $output.='style="color:'.esc_attr($plan_price_color).'"'; } $output.='>'.esc_attr( $plan_price).'</span>
	'; } $output.='
	'; if($plan_duration){ $output.='
	<span class="font18" '; if($plan_duration_color){ $output.='style="color:'.esc_attr($plan_duration_color).'"'; } $output.='>'.esc_attr( $plan_duration).'</span>
	'; } $output.='</div>
	<ul class="fea-lists pl-4 pr-4">'; 	
	$c=-1;
	foreach($plan_all_features as $feature) {
	$c++;
	$output .= '<li '; if($plan_feature_color){ $output.='style="color:'.esc_attr($plan_feature_color).'"'; } $output.='>'.strip_tags($feature).'</li>';
	} $output.='</ul>  
	'; if($plan_btn_name){ $output.='
   <div class="mt-4">
	<a href="'.esc_url( $plan_btn_url).'"  class="outline-button" '; if($plan_btn_text_color || $plan_btn_bg_color){ $output.='style="'; if($plan_btn_text_color){ $output.='color:'.esc_attr($plan_btn_text_color).';border-color:'.esc_attr($plan_btn_text_color).';'; } if($plan_btn_bg_color){ $output.='background-color:'.esc_attr($plan_btn_bg_color).''; } $output.='"'; } $output.='>
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.construction_pricing_table.'.wp_trim_words($unique_css_class).' .outline-button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
		border-color:'.esc_attr($btn_hover_text_color).' !important;
	}	
	</style>';
}
...
add_shortcode('ronby_shortcodes_for_construction_pricing_table', 'ronby_construction_pricing_table_shortcode');

/*****************************
Construction Projects Tab Section-2
******************************/
//Function for Construction Projects Tab Section-2
function ronby_construction_projects_tab_sec_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
    'showall' => '',
	'features' => '',
	'number_of_items' => '',
	...
	if(!empty($features)){
      $output = '<div class="mb-4" '; if($tab_margin_left){ $output.='style="margin-left:'.esc_attr($tab_margin_left).'"'; } $output.='>
						<div class="filter-nav-6">	  
							<div id="filters">
				  <ul class="list-unstyled pl-0">
				  '; if($showall){ $output.='
					<li  class="tab active-color-primary hover-color-primary animate-300 is-checked active cursor-point" data-filter="*">
						<span class="text">'.esc_attr($showall).'</span>
				  </li>';
				  }
      $features = !empty($features) ? explode("\n", trim($features)) : array(); 
	  ...
	  $feature = strip_tags($feature);
	  $c++;
      $output .= '<li class="tab active-color-primary hover-color-primary animate-300 cursor-point" data-filter=".'.$feature.'">';
	...
	$out = '<section class="construction-our-projects-2 '.esc_attr($unique_css_class).' p-30-0-30">';
	$out .= ''.do_shortcode($content).'';$out .= '
    <div class="tabs-filter tabs-filter-default d-md-flex">';
	$out .= '<div class="flex-fill">
			<div class="grid">';
							$i=1;
							$c=0;
							...
							$item_url =$b['item_url'.$c.''];
							$item_features =$b['item_features'.$c.''];
$out .='<div class="col-md-3 element-item nopadding '.esc_attr( $item_features ).'" data-category="'.esc_attr( $item_features ).'">
		<div class="article-with-overlay-4 mb-0">
		'; if($item_img){$out.='
			<a href="'.esc_url($item_url).'" class="thumbnail">
				<img src="'.esc_url($item_img).'" alt="'.esc_attr__('featured-image','ronby').'">
			</a>
  ...
  $out.='<script>
  jQuery(function() {       
  jQuery(".filter-nav-6 li").click(function() {   
		jQuery(".filter-nav-6 li").removeClass("active");      
		jQuery(this).addClass("active");      
	  });
	});
  </script>';
...
if($active_tab_color){
	$out.='
	<style>
	.construction-our-projects-2.'.wp_trim_words($unique_css_class).' .active-color-primary.active{
		color:'.esc_attr($active_tab_color).' !important;
	}	
	.construction-our-projects-2.'.wp_trim_words($unique_css_class).' .active-color-primary:hover{
		color:'.esc_attr($active_tab_color).' !important;
	}	
	</style>';
} 
if($box_hover_color){
	$out.='
	<style>
	.construction-our-projects-2.'.wp_trim_words($unique_css_class).' .article-with-overlay-4:hover .overlay{
		background-color:'.esc_attr($box_hover_color).' !important;
	}		
	</style>';
}
...
if($hover_icon_color || $hover_icon_bg || $hover_text_color){
	...
	<style>
	'; if($hover_icon_color || $hover_icon_bg){$out.='
	.construction-our-projects-2.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .icon{
		color:'.esc_attr($hover_icon_color).' !important;
		background-color:'.esc_attr($hover_icon_bg).' !important;
	}
	'; } if($hover_text_color){ $out.='
	.construction-our-projects-2.'.wp_trim_words($unique_css_class).' .article-with-overlay-4 .post-title{
		color:'.esc_attr($hover_text_color).' !important;
	}
	'; }$out.='
	</style>';
...
add_shortcode('ronby_shortcode_for_construction_projects_tab_section_two', 'ronby_construction_projects_tab_sec_two_shortcode');

/********************************
Restaurant Products Tab Section-2
*********************************/
//Function for Restaurant Products Tab Section-2
function ronby_restaurant_products_tab_section_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
    'showall' => '',
	'showall_icon' => '',
	'features' => '',
...
$bgimage = wp_get_attachment_image_src( $explode_bg_img_id, 'bg_img' );
	...
	$out .='<div class="col-md-3 element-item text-center '.esc_attr( $item_features ).'" data-category="'.esc_attr( $item_features ).'">
						<article class="product-item-2 thumb-right-style">
							<div class="d-sm-flex align-items-center">
								...
								'; }$out.='	
									'; if($item_rating){$out.='
										<div class="stars-rating d-inline-block mt-4" data-rate="5">
										'; if($item_rating == '1'){$out.='
											<span class="fas fa-star"></span>
										'; } elseif($item_rating == '2'){$out.='	
											<span class="fas fa-star"></span>
								...
								'; if($item_title){$out.='
									<a href="'.esc_url( $more_url ).'" class="no-color">
										<h3 class="product-name mt-2" '; if($title_color){$out.='style="color:'.esc_attr($title_color).'"'; }$out.='>'.esc_attr( $item_title ).'</h3>
									</a>
								'; }$out.='	
								'; if($item_price){$out.='
									<div class="text-center" '; if($price_color){$out.='style="color:'.esc_attr($price_color).'"'; }$out.='>
										'.esc_attr( $item_price ).'
									</div>
								'; }$out.='								
									<div class="product-meta">
									'; if($item_date){$out.='
										<span class="product-date" '; if($date_color){$out.='style="color:'.esc_attr($date_color).'"'; }$out.='>'.esc_attr($item_date).'</span>
									'; }$out.='	

									</div>
									'; if($text){$out.='
									<div class="product-description" '; if($desc_color){$out.='style="color:'.esc_attr($desc_color).'"'; }$out.='>
										'.esc_attr($text).'
									</div>
									'; }$out.='	
								</div>
...
add_shortcode('ronby_shortcode_for_ronby_restaurant_products_tab_section_two', 'ronby_restaurant_products_tab_section_two_shortcode');

/*********************************
Restaurant Products Tab Section-3
******************************/
//Function for Restaurant Products Tab Section-3
function ronby_restaurant_products_tab_section_three_shortcode( $atts ) {
	extract( shortcode_atts( array(
    'showall' => '',
	'showall_icon' => '',
	'features' => '',
...
$bgimage = wp_get_attachment_image_src( $explode_bg_img_id, 'bg_img' );
	...
	$out = '<section class="restaurant_products_tab_3 r-product-tabs" '; if($bgimage[0] || $padding_top || $padding_bottom){$out.='style="'; if($bgimage[0]){$out.='background:url('.esc_url($bgimage[0]).') no-repeat;background-position: left bottom 100px;'; } if($padding_top){$out.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){$out.='padding-bottom:'.esc_attr($padding_bottom).';'; }$out.='"'; }$out.='>
    <div class="container">';
    $out .= '  '.do_shortcode($content).'';
	$out .= '<div class="grid">';
							$i=1;
	...
	$out .='<div class="col-md-6 element-item text-center '.esc_attr( $item_features ).'" data-category="'.esc_attr( $item_features ).'">
						<article class="product-item-2 thumb-right-style">
							<div class="d-sm-flex align-items-center">
								...
								'; if($item_title){$out.='
									<a href="'.esc_url( $more_url ).'" class="no-color d-inline float-left item-title" '; if($itemtitlepricebg){$out.='style="background-color:'.esc_attr($itemtitlepricebg).'"'; }$out.='>
										<h3 class="product-name mt-2">'.esc_attr( $item_title ).'</h3>
									</a>
								'; }$out.='	
								'; if($item_price){$out.='
									<div class="d-inline float-right item-price" '; if($itemtitlepricebg){$out.='style="background-color:'.esc_attr($itemtitlepricebg).'"'; }$out.='>
										'.esc_attr( $item_price ).'
									</div>
								'; }$out.='	
								</div>	
									...
									'; if($text){$out.='
									<div class="product-description d-inline-block">
										'.esc_attr($text).'
									</div>
									'; }$out.='
									<div class="product-meta d-inline-block">
									'; if($item_date){$out.='
										<span class="product-date">'.esc_attr($item_date).'</span><br>
									'; }$out.='	
									'; if($item_rating){$out.='
...
add_shortcode('ronby_shortcode_for_ronby_restaurant_products_tab_section_three', 'ronby_restaurant_products_tab_section_three_shortcode');

/*****************************
Restaurant Food Gallery- 2
******************************/
//Function for Restaurant Food Gallery- 2
function ronby_restaurant_food_gallery_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'number_of_items' => '',
	'title1_color' => '',
	'title2_color' => '',
	...
	$output='<section class="p-30-0-30 restaurant_food_gallery_two">';	
	$output .='<div class="">';
	$output .='<div class="masonry grid masonry-no-gutter masonry-5-columns">
				<div class="masonry-sizer"></div>
				<div class="masonry-gutter"></div>
					...
					$item_features =$b['item_features'.$c.''];
					$large_width_img =$b['large_width_img'.$c.''];
					$itemprice =$b['itemprice'.$c.''];
					$output .='				
				<div class="masonry-item  '; if($large_width_img == "yes"){ $output.='masonry-item-width-2x'; } $output.='" >
					<article class="article-with-overlay-3">
					'; if($img_url){ $output.='
						<div class="thumbnail">
							<img src="'.esc_url($img_url).'" alt="'.esc_attr__('featured-image','ronby').'"/>
					...
					'; } $output.='	
						<div class="overlay overlay-fill d-flex flex-column justify-content-end">	
							...
							<div class="item-text row p-2 pt-3">
							
							<div class="col-sm-8">
							'; if($headlineone){ $output.='
								<h5 class="item-title  color-primary float-left d-inline-block" '; if($title1_color){ $output.='style="color:'.esc_attr($title1_color).'"'; } $output.='>'.esc_attr($headlineone).'</h5>								
							'; } $output.='
							</div>
							<div class="col-sm-4">
							'; if($itemprice){ $output.='
								<h5 class="item-price color-primary desktop-float-right d-inline-block" '; if($itemprice_color){ $output.='style="color:'.esc_attr($itemprice_color).'"'; } $output.='>'.esc_attr($itemprice).'</h5>								
							'; } $output.='
							</div>	
							<div class="border-bottom-dashed"></div>
							'; if($headlinetwo){ $output.='
							<div class="col-sm-12">
								<p class="item-sub-title d-inline-block mt-1" '; if($title2_color){ $output.='style="color:'.esc_attr($title2_color).'"'; } $output.='>'.esc_attr($headlinetwo).'</p>
							</div>	
							'; } $output.='	
							</div>
				</div>
...
add_shortcode('ronby_shortcode_for_restaurant_food_gallery_two', 'ronby_restaurant_food_gallery_two_shortcode');

/*****************************
Fashion Woocommerce Products Section-2
******************************/
//Function for Fashion Woocommerce Products Section-2
function ronby_fashion_woo_products_section_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'cat_id'      => '',
	'post_per_page' => '-1',
	'order'   => 'desc',
	...
	$wp_query= null;			
	$wp_query = new WP_Query($args);
	$output='<section class="section-feature-products '.esc_attr($unique_css_class).' p-30-0-30 fashion-section-feature-products-2">
	<div class="container">
				<div class="row">';
	if ($wp_query->have_posts()) :  while ($wp_query->have_posts()) : $wp_query->the_post();
			global $product;
	...
	if( $product->is_on_sale() ){ 
	$output.= '<div class="ribbon">
				<span class="ribbon2 text-white">S<br>A<br>L<br>E</span>
			</div>';
	}
			$output.='<div class="thumbnail animate-zoom d-flex" '; if($box_bg_color || $middle_border_color){ $output.='style="'; if($box_bg_color){ $output.='background-color:'.esc_attr($box_bg_color).';'; } if($middle_border_color){ $output.='border-bottom:1px solid '.esc_attr($middle_border_color).''; };$output.='"'; } $output.='>
				<a href="'.esc_url(get_permalink($id)).'">';
			...
			$output.='</a>
					...
					<div class="shadow-brdr"><div class="item-buttons product-action-buttons-1 d-flex justify-content-center v-hidden">';
   			if($img_url){
				$output.='<a href="'.esc_url($img_url).'" class="hover-background-primary cursor-point" data-lightbox="featured-image">
					<i class="fas fa-search"></i></a>';
   			} else {
...
if($badge_bg_before && $badge_bg_after){
	$output.='
	<style>
	.fashion-section-feature-products-2.'.wp_trim_words($unique_css_class).'  .ribbon-wrap::before{
		background-color:'.esc_attr($badge_bg_before).';
	}	
	.fashion-section-feature-products-2.'.wp_trim_words($unique_css_class).'  .ribbon-wrap::after{
		background-color:'.esc_attr($badge_bg_after).';
	}	
	</style>';
}
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.fashion-section-feature-products-2.'.wp_trim_words($unique_css_class).'  .item-buttons a:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
	...
	<script>
	jQuery (document).ready(function(){
	jQuery(".fashion-section-feature-products-2 .product-item-1 .item-buttons").addClass("v-hidden");
	
    jQuery(".fashion-section-feature-products-2 .product-item-1").hover(function() {
...
add_shortcode('ronby_shortcode_for_fashion_woo_products_two_section', 'ronby_fashion_woo_products_section_two_shortcode');

/*****************************
Fashion Category Slider
******************************/
//Function for Fashion Category Slider
function ronby_fashion_category_slider_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'bg_color' => '',
	'text_color' => '',
	'name_color' => '',
...
$count=count($explode);
	...
	$output='<section class="business-steps blog-page-business-steps  fashion_category_slider '.esc_attr($unique_css_class).'" '; if($bg_color){ $output.='style="background-color:'.esc_attr($bg_color).'"'; } $output.='>
				<div class="fashion-cat-slider owl-carousel">';
	$i=1;
	$c=0;
	while ($i<=$count){

	$catname= get_the_category_by_ID($cat_id[$c]); 
	$output.='<div class="item">
				<div class="article-with-overlay-1">
					<div class="thumbnail animate-zoom">
						<div style="background-image:url('; if(!empty(get_option('ronby_taxonomy_image'.$cat_id[$c]))){ $output.= esc_url(get_option('ronby_taxonomy_image'.$cat_id[$c])); } else{ $output.=esc_url(get_template_directory_uri()."/images/placeholder.png"); } $output.=');" class="blog-cat-f-section"></div>
...
add_shortcode('ronby_shortcode_for_fashion_category_slider', 'ronby_fashion_category_slider_shortcode');

/******************* 
Restaurant Food Category- 2
********************/
//This function is for Restaurant Food Category- 2
function ronby_restaurant_food_category_two_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' 		 => '',
	'heading2' 		 => '',
	'multiple_cat' 		 => '',
	...
	$output ='<section class="section-category-of-food section-category-of-food-2" '; if($left_bottom_image[0] || $right_top_image[0]){ $output.='style="background:'; if($left_bottom_image[0]){ $output.='url('.esc_url($left_bottom_image[0]).') no-repeat left bottom,'; } if($right_top_image[0]){ $output.='url('.esc_url($right_top_image[0]).') no-repeat right top;'; } if($padding_top){ $output.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){ $output.='padding-bottom:'.esc_attr($padding_bottom).';'; } $output.='"'; } $output.='>
			<div class="container">
			'; if($heading1 || $heading2){ $output.='
				<div class="section-header-style-15">
				'; if($heading1){ $output.='
					...
					<div class="masonry-item">
						<article class="product-item-3">
							<div class="thumbnail">
								...
								<a class="no-color" href="'.esc_url(get_category_link($cat_id)).'">
									<div class="overlay d-flex flex-column justify-content-end">							<div class="title_wrapper">	
										<h3 class="product-name">
											'.esc_attr(get_the_category_by_ID($cat_id)).'
										</h3>
										<div class="text-center">
											...
											<div class="product-item-stats d-inline-block">
												'; if($product_term){ $output.=''.esc_attr($product_term->count).''; } $output.=' '.esc_attr($product_type).'
											</div>
										</div>
										</div>		
									</div>
...
add_shortcode('ronby_shortcode_for_restaurant_food_category_two', 'ronby_restaurant_food_category_two_shortcode');

/********************************
Restaurant Products Slider
*********************************/
//Function for Restaurant Products Slider
function ronby_restaurant_products_slider_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'number_of_items' => '',
	'bg_img' => '',
	'padding_top' => '',
...
$bgimage = wp_get_attachment_image_src( $explode_bg_img_id, 'bg_img' );
	...
	$out = '<section class="r-product-tabs" '; if($bgimage[0] || $padding_top || $padding_bottom){$out.='style="'; if($bgimage[0]){$out.='background:url('.esc_url($bgimage[0]).') no-repeat;background-position: left bottom 100px;'; } if($padding_top){$out.='padding-top:'.esc_attr($padding_top).';'; } if($padding_bottom){$out.='padding-bottom:'.esc_attr($padding_bottom).';'; }$out.='"'; }$out.='>
    <div class="container"><div class="restaurant-product-slider '.esc_attr($unique_css_class).' owl-carousel">';
	...
	$out .='<div class="item"><div class="col-md-12  text-center '.esc_attr( $item_features ).'" data-category="'.esc_attr( $item_features ).'">
						<article class="product-item-2 thumb-right-style">
							<div class="d-sm-flex align-items-center">
								<div class="mb-4 mb-sm-0">	
								'; if($item_img){$out.='
  ...
  if($slider_dot_bg){
	$out.='
	<style>
	.restaurant-product-slider.'.wp_trim_words($unique_css_class).' .owl-dot.active span{
		background-color:'.esc_attr($slider_dot_bg).' !important;
	}	
	.restaurant-product-slider.'.wp_trim_words($unique_css_class).'  .owl-dot.active{
		border-color:'.esc_attr($slider_dot_bg).' !important;		
	}	
	</style>';
}
...
add_shortcode('ronby_shortcode_for_restaurant_products_slider', 'ronby_restaurant_products_slider_shortcode');

/******************
Portfolio Slider-1
******************/
function ronby_portfolio_slider_one_shortcode( $atts, $content = null ) {
    extract(shortcode_atts(array(	
	'heading1'=>'',	
	'heading2'=>'',	
	'desc'=>'',	
    ...
    $output ='
	<!-- portfolio seciton start -->
    <section class="blasa-section portfolio-section-3  '.esc_attr($unique_css_class).' portfolio-slider-one" >
        <div class="container">
            <div class="row align-items-end justify-content-center row-portfolio-head">
                <div class="col-lg-5 col-md-12 col-12">
                    <div class="section-head b-align-left tp_animate_when_visible fade-bottom" data-offest="80%" data-speed="600">
                        <span class="small-title">'.esc_attr($heading1).'</span>
                        <h2 class="head-title">'.$heading2.'</h2>
                    </div>
                </div>
                <div class="col-lg-7 col-md-12 col-12">
                    <p class="col-portfoli-desc tp_animate_when_visible fade-bottom" data-offest="80%" data-delay="200" data-speed="600">'.$desc.'</p>
                </div>
            </div>
<div class="portfolio-owl owl-carousel portfolio-1" data-show="3">
';
	$i=1;
	$c=0;
	while ($i<=$a['number_of_portfolio']){
	$c++;
	$b = shortcode_atts(array(
	'title'.$c.'' => '',
	'image'.$c.'' => '',
	...
	$title =$b['title'.$c.''];
	$image =$b['image'.$c.''];
	$onclickurl =$b['onclickurl'.$c.''];
	$output .='			
			

                <div class="wrap-project">
                    <div class="wrap-case">
                        <a href="'.esc_url($onclickurl).'" title="">
                        <div class="case_overlay">
                            <div class="inner_case_overlay"></div>
                        </div>
                        <div class="case_wrap_content">
                            <div class="case_t_overlay">
                                <div class="wrap_case_title">
                                    <h3>'.esc_attr($title).'</h3>
                                </div>
                            </div>
                        </div>
                        <div class="wrap_case_image" style="background-image:url('.esc_url($image).');">
                        </div>
                    </a>
                    </div>
                </div>
...
$output .='				
	...
	<!-- portfolio seciton end -->
    <div class="clearfix"></div>	
	';
	
echo'<style>
'; if(!empty($heading1_color)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .small-title{
color:'.esc_attr($heading1_color).';
}
'; } echo'
'; if(!empty($heading2_color)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .head-title{
color:'.esc_attr($heading2_color).' !important;
}
'; } echo'
'; if(!empty($desc_color)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .col-portfoli-desc{
color:'.esc_attr($desc_color).' !important;
}
'; } echo'
'; if(!empty($bg_color)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).'{
background-color:'.esc_attr($bg_color).' !important;
}
'; } echo'
'; if(!empty($bg_color2)){ echo' 
.portfolio-slider-one.blasa-section.portfolio-section-3.'.wp_trim_words($unique_css_class).'::before{
	background-color:'.esc_attr($bg_color2).' !important;
}
'; } echo'
'; if(!empty($bg_img)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .portfolio-1::before{
background-image:url('.esc_url($bg_img).');
}
'; } echo'
'; if(!empty($btn_bg_color)){ echo' 
.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .portfolio-owl .owl-nav button,.portfolio-slider-one.'.wp_trim_words($unique_css_class).' .portfolio-owl .owl-dots button.active{
background-color:'.esc_attr($btn_bg_color).' !important;
}
'; } echo'
'; if(!empty($btn_bg_color)){ echo' 
.portfolio-slider-one .portfolio-owl .owl-dots button.active::before{
    border-bottom-color: '.esc_attr($btn_bg_color).' !important;
}
'; } echo'
</style>';
...
add_shortcode('ronby_shortcode_for_portfolio_slider_one', 'ronby_portfolio_slider_one_shortcode');

/******************
Service Section- 9
******************/
function ronby_business_service_sec_two_shortcode( $atts, $content = null ) {
    extract(shortcode_atts(array(
	'heading1'=>'',
	'heading2'=>'',
	'heading3'=>'',
    ...
    $output ='
    <!-- service section -->
    <section class="blasa-section service-section '.esc_attr($unique_css_class).' service-section-nine" >
        <div class="container">
            <div class="row row-services-head align-items-end">
                <div class="col-12">
                    <div class="section-head b-align-center tp_animate_when_visible fade-top" data-speed="600" data-offest="90%">
                        <span class="small-title">'.wp_kses(__($heading1),$allowed_html_array).'</span>
                        <h2 class="head-title">'.wp_kses(__($heading2),$allowed_html_array).'</h2>
                         <div class="our_service_text tp_animate_when_visible fade-top" data-speed="600" data-delay="200" data-offest="90%">
                        <p>'.wp_kses(__($heading3),$allowed_html_array).'</p>
                    </div>
                    </div>
                </div>
			...
			'; if(!empty($icon1) && !empty($title1) && !empty($desc1)){ $output.='
                <div class="col-lg-4 col-md-12 col-12">
                    <div class="row row_inner_services">
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12 ">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="700" data-offest="75%">
                                <span class="service-icon"><i class="'.esc_attr($icon1).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title1).'</h5>
                                    <p>'.$desc1.'</p>
                                </div>
                            </div>
                        </div>
			'; } if(!empty($icon2) && !empty($title2) && !empty($desc2)){ $output.='
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="700" data-offest="75%" data-delay="100">
                                <span class="service-icon"><i class="'.esc_attr($icon2).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title2).'</h5>
                                    <p>'.$desc2.'</p>
                                </div>
                            </div>
                        </div>
			...
			'; } if(!empty($icon3) && !empty($title3) && !empty($desc3)){ $output.='	
                <div class="col-lg-4 col-md-12 col-12">
                    <div class="row row_inner_services right_irow_services">
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="700" data-offest="75%" data-delay="200">
                                <span class="service-icon"><i class="'.esc_attr($icon3).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title3).'</h5>
                                    <p>'.$desc3.'</p>
                                </div>
                            </div>
                        </div>
			'; } if(!empty($icon4) && !empty($title4) && !empty($desc4)){ $output.='			
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="700" data-offest="75%" data-delay="300">
                                <span class="service-icon"><i class="'.esc_attr($icon4).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title4).'</h5>
                                    <p>'.$desc4.'</p>
                                </div>
                            </div>
                        </div>
				...
				'; } if(!empty($icon5) && !empty($title5) && !empty($desc5)){ $output.='
                <div class="col-lg-4 col-md-12 col-12">
                    <div class="row row_inner_services right_irow_services">
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12 ">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="700" data-offest="75%" data-delay="400">
                                <span class="service-icon"><i class="'.esc_attr($icon5).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title5).'</h5>
                                    <p>'.$desc5.'</p>
                                </div>
                            </div>
                        </div>
				'; } if(!empty($icon6) && !empty($title6) && !empty($desc6)){ $output.='		
                        <div class="col-lg-12 col-md-6 col-sm-12 col-12 ">
                            <div class="wrap_service tp_animate_when_visible fade-bottom" data-speed="600" data-offest="75%" data-delay="500">
                                <span class="service-icon"><i class="'.esc_attr($icon6).'"></i></span>
                                <div class="wrap_service_txt">
                                    <h5>'.esc_attr($title6).'</h5>
                                    <p>'.$desc6.'</p>
                                </div>
                            </div>
                        </div>
...
echo'<style>
'; if(!empty($heading1_color)){ echo' 
.service-section-nine.'.wp_trim_words($unique_css_class).' .small-title{
color:'.esc_attr($heading1_color).';
}
'; } echo'
'; if(!empty($heading2_color)){ echo' 
.service-section-nine.'.wp_trim_words($unique_css_class).' .head-title{
color:'.esc_attr($heading2_color).';
}
'; } echo'
'; if(!empty($heading3_color)){ echo' 
.service-section-nine.'.wp_trim_words($unique_css_class).' .our_service_text{
color:'.esc_attr($heading3_color).';
}
'; } echo'
'; if(!empty($icon_color) || !empty($icon_bg_color)){ echo' 
.service-section-nine.'.wp_trim_words($unique_css_class).' .service-icon{
color:'.esc_attr($icon_color).';
background-color:'.esc_attr($icon_bg_color).';
}
'; } echo'
'; if(!empty($icon_hover_color) || !empty($icon_hover_bg_color)){ echo' 
.service-section-nine.'.wp_trim_words($unique_css_class).' .wrap_service:hover .service-icon{
...
add_shortcode('ronby_shortcode_for_business_service_sec_two', 'ronby_business_service_sec_two_shortcode');

/******************
Business About us Section -4
******************/
function ronby_business_about_us_section_four( $atts, $content = null ) {
    extract(shortcode_atts(array(
	'heading1'=>'',
	'heading2'=>'',
	'desc'=>'',
    ...
    $output ='
    <section class="blasa-section about_us_1  '.esc_attr($unique_css_class).' about_us_eight" >
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-6  col-about-img">
                    <div class="wrap_about_img"></div>
                </div>
                <div class="col-lg-5 ">
                    <div class="wrap_about_1_content">
                        <span class="small-title tp_animate_when_visible fade-left" data-offest="70%" data-speed="600" data-delay="500">'.esc_attr($heading1).'</span>
                        <h2 class="head-title tp_animate_when_visible fade-bottom" data-offest="70%" data-speed="700">'.esc_attr($heading2).'</h2>
                        <p class="tp_animate_when_visible tp-fade" data-offest="70%" data-delay="300" data-speed="600">'.wp_kses(__($desc),$allowed_html_array).'</p>
                        <div class="about_us_btn tp_animate_when_visible fade-left" data-offest="90%" data-delay="400" data-speed="700">
                            <a href="'.esc_url($btn_url).'" title="about us">'.esc_attr($btn_name).' <i class="fas fa-chevron-right"></i></a>
                        </div>
                    </div>
                </div>
            </div>
...
$output.='<style>
'; if(!empty($bg_image)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' .wrap_about_img{
    background-image:url('.esc_url($bg_image).');
}
'; } $output.='
'; if(!empty($heading1_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' .small-title{
    color:'.esc_attr($heading1_color).';
}
'; } $output.='
'; if(!empty($heading2_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' .head-title {
    color:'.esc_attr($heading2_color).';
}
'; } $output.='
'; if(!empty($desc_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' p{
    color:'.esc_attr($desc_color).';
}
'; } $output.='
'; if(!empty($btn_text_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' a{
    border: 1px solid '.esc_attr($btn_text_color).';
    color: '.esc_attr($btn_text_color).';
}
'; } $output.='
'; if(($btn_text_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' a i{
    background: '.esc_attr($btn_text_color).';
}
'; } $output.='
'; if(!empty($btn_hover_text_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' a:hover{
	background-color: '.esc_attr($btn_hover_text_color).';
}
'; } $output.='
'; if(!empty($btn_hover_text_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' a:hover i{
	color: '.esc_attr($btn_hover_text_color).';
}
'; } $output.='
'; if(!empty($bg_color)){ $output.='
.about_us_eight.'.wp_trim_words($unique_css_class).' {
	background-color: '.esc_attr($bg_color).' !important;
}
'; } $output.='
</style>';
...
add_shortcode('ronby_shortcode_for_business_about_us_section_four', 'ronby_business_about_us_section_four');


/*****************************
Shop Products Section
******************************/
//Function for Shop Products Section
function ronby_fitness_woo_products_section_shortcode_2( $atts ) {
	extract( shortcode_atts( array(
	'cat_id'      => '',
	'post_per_page' => '-1',
	'order'   => 'desc',
	...
	$wp_query= null;			
	$wp_query = new WP_Query($args);
	$output='<section class="section-feature-products '.esc_attr($unique_css_class).' p-30-0-30 fitness-section-feature-products fitness-section-feature-products-2">
	<div class="container">
				<div class="row">';
	if ($wp_query->have_posts()) :  while ($wp_query->have_posts()) : $wp_query->the_post();
			global $product;
	...
	if($ronby_product_new_badge) { 
	$output.= '<div class="sale-badge" '; if($ronby_product_new_badge_bg || $badge_text_color || $dashed_border){ $output.='style="'; if($ronby_product_new_badge_bg){ $output.='background-color:'.esc_attr($ronby_product_new_badge_bg).';'; } else { if($badge_bg){ $output.='background-color:'.esc_attr($badge_bg).';'; } } if($badge_text_color){ $output.='color:'.esc_attr($badge_text_color).';'; } if($dashed_border=="yes"){ $output.='border:1px dashed;'; } $output.='"'; } $output.='>
				<span>'.esc_attr($ronby_product_new_badge).'</span>
			</div>';
	}
			$output.='<div class="thumbnail animate-zoom d-flex" '; if($box_bg_color || $middle_border_color){ $output.='style="'; if($box_bg_color){ $output.='background-color:'.esc_attr($box_bg_color).';'; } if($middle_border_color){ $output.='border-bottom:1px solid '.esc_attr($middle_border_color).''; };$output.='"'; } $output.='>
				<a href="'.esc_url(get_permalink($id)).'">';
			...
			$output.='</a>
					...
					<div class="shadow-brdr"><div class="row ml-0 mr-0">';
			
					$output.='<div class="col-md-7 text-left pt-3 pb-3">';
					if($average){
					$output.= '<div class="stars-rating justify-content-center" data-rate="5">';
					while($i < $average){
						$output.="<span class='fas fa-star'></span>";
					...
					$output.=ronby_fitness_add_to_cart_button_two();				
					$output.='</div>
...
add_shortcode('ronby_shortcode_for_ronby_fitness_woo_products_section_2', 'ronby_fitness_woo_products_section_shortcode_2');

/*****************************
Business Portfolio Section
******************************/
//Function for Business Portfolio Section
function ronby_business_portfolio_sec_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading2' => '',
	'desc' => '',
	...
	$output='<!-- Section header -->
	<section class="business-portfolio-sec '.esc_attr($unique_css_class).'">
		<div class="container">
		<div class="row">
			<div class="col-md-6">
			<div class="construction-heading-section section-header-style-4  mb-50">	
			'; if($heading1){ $output.='
				<h4 class="section-sub-title" '; if($heading1_color){ $output.='style="color:'.esc_attr($heading1_color).'"'; } $output.='>'.esc_attr($heading1).'</h4>
			'; } $output.='
			'; if($heading2){ $output.='
				<h1 class="section-title mt-3" '; if($heading2_color){ $output.='style="color:'.esc_attr($heading2_color).'"'; } $output.='>'.$heading2.'</h1> 
			'; } $output.='
			'; if($desc) { $output.='
				<p class="mt-3" '; if($desc_color) { $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>'.$desc.'</p>	
			'; } $output.='
				</div>';
	...
	$i=1;
	$c=0;
	while ($i<=$a['number_of_progress_bar']){
	$c++;
	$b = shortcode_atts(array(
		'value'.$c.'' => '',
		'title'.$c.'' => '',
	...
	$output .='<div class="progress progressbar-style-2" '; if($progress_bar_bg_color){ $output.='style="background-color:'.esc_attr($progress_bar_bg_color).';height:3px"' ; } $output.='>
              <div class="progress-bar" role="progressbar" aria-valuenow="'.esc_attr($value).'" aria-valuemin="0" aria-valuemax="100" '; if($progress_bar_color){ $output.='style="background-color:'.esc_attr($progress_bar_color).';width: 80%;color:'.esc_attr($title_color).';height:3px"' ; } $output.='>  <span class="text-gyellow">'.esc_attr($title).' '.esc_attr($value).'%</span> </div>
            </div>';
	$i++;
	}
	$output .='<!--end bar-->				
			</div>
			<div class="col-md-6 bg-vdo-sec-one">
			'; if($img_url) { $output.='
            <div class="row_processes_head" style="background-image:url('.esc_url($img_url).');background-size: cover;">
                <a href="'.esc_url($youtube_vdo_link).'" class="rq-play-video tp_animate_when_visible fade-bottom popup-youtube" data-offest="80%" target="_blank"><i class="fas fa-play" ></i></a>
            </div>
			'; } $output.='				
			</div>	
			</div>	
...
if($border_width || $border_bg_color){
	$output.='
	<style>
	';if($border_width) { $output.='
	.business-portfolio-sec.'.wp_trim_words($unique_css_class).'  .section-sub-title::after{
		width:'.esc_attr($border_width).';
	}	
	'; } $output.='
	';if($border_bg_color) { $output.='
	.business-portfolio-sec.'.wp_trim_words($unique_css_class).'  .section-sub-title::after{
		background-color:'.esc_attr($border_bg_color).';
	}
	'; } $output.='	
	</style>';
}
...
add_shortcode('ronby_shortcode_for_business_portfolio_sec', 'ronby_business_portfolio_sec_shortcode');

/*****************************
Business About us section- 5
******************************/
//Function for Business About us section- 5
function ronby_business_about_us_sec_five_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'bg_img' => '',
	'heading' => '',
	'description' => '',
	...
	$output='
	<section class="section-side-image '.esc_attr($unique_css_class).' section-dark clearfix" '; if($bg_color) { $output.='style="background-color:'.esc_attr($bg_color).'"'; } $output.='>
		      <div class="img-holder col-md-6 col-sm-3 pull-left">
			  '; if($bg_img) { $output.='
		        <div class="background-imgholder" style="background:url('.esc_url($bg_img).');"><img class="nodisplay-image" src="'.esc_url($bg_img).'" alt="'.esc_attr__('featured-image','ronby').'"> </div>
			  '; } $output.='
		      </div>
		      <div class="container-fluid">
		        <div class="row">
		              ...
		              <div class="ce3-feature-box-7">
		                <div class="col-xs-12 nopadding">
		                  <div class="sec-title-container less-padding-3 text-left">
		                    <div class="ce4-title-line-1 align-left"></div>
							'; if($heading) { $output.='
		                    <h4 class="uppercase font-weight-7 less-mar-1 text-white" '; if($heading_color) { $output.='style="color:'.esc_attr($heading_color).' !important"'; } $output.='>'.esc_attr($heading).'</h4>
							'; } $output.='
		                    <div class="clearfix"></div>
							'; if($description) { $output.='
		                    <p class="by-sub-title" '; if($desc_color) { $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>'.$description.'</p>
							'; } $output.='
		                  </div>
		                </div>
		                <div class="clearfix"></div>
		                <!--end title-->'; 	
							...
							foreach($features as $feature) {
							$output.='<div class="iconlist-2" '; if($features_color) { $output.='style="color:'.esc_attr($features_color).'"'; } $output.='>';
							$output.='<div class="icon dark"><i class="fas fa-long-arrow-alt-right"></i></div>';
							$output.='<div class="text">'.strip_tags($feature).'</div>';
							$output.='</div>';
							} $output.='						  
		                <!--end item-->
		                ...
		                <br>
		                <br>
		                <a class="button button-secondary" href="'.esc_url($btn_url).'">'.esc_attr($btn_label).'</a></div>
		            </div>
		          </div>
		        </div>
		      </div>
...
if($btn_text_color && $btn_bg_color){
	$output.='
	<style>
	.section-side-image.'.wp_trim_words($unique_css_class).' .button{
		color:'.esc_attr($btn_text_color).' !important;
		background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.section-side-image.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
add_shortcode('ronby_shortcode_for_business_about_us_sec_five', 'ronby_business_about_us_sec_five_shortcode');

/******************
Business Feature Box Section- 3
******************/
function ronby_business_feature_box_sec_3_shortcode( $atts, $content = null ) {
    extract(shortcode_atts(array(
	'icon'=>'',
	'title'=>'',
	'desc'=>'',
	...
	<section class="'.esc_attr($unique_css_class).' business-feature-box-3 p-30-0-30" >
	<div class="wrap_about_feature b-align-center">
                        <span class="about_ficon_2 tp_animate_when_visible zoom-in" data-offest="75%"><i class="'.esc_attr($icon).'"></i></span>
                        <h3>'.esc_attr($title).'</h3>
                        <p class="about_feature_text ">
							'.esc_attr($desc).'
                        </p>
    </div>
	</section>
	...
	<div class="clearfix"></div>
	';
if($icon_bg_color || $title_color || $desc_color){
echo'<style>
'; if($icon_bg_color){ echo' 
.business-feature-box-3.'.wp_trim_words($unique_css_class).' .about_ficon_2{
	background-color:'.esc_attr($icon_bg_color).';
}
'; } echo'
'; if($title_color){ echo' 
.business-feature-box-3.'.wp_trim_words($unique_css_class).' .wrap_about_feature h3{
	color:'.esc_attr($title_color).';
}
'; } echo'
'; if($desc_color){ echo' 
.business-feature-box-3.'.wp_trim_words($unique_css_class).' p{
	color:'.esc_attr($desc_color).';	
}
'; } echo'
</style>';
...
add_shortcode('ronby_shortcode_for_business_feature_box_sec_3', 'ronby_business_feature_box_sec_3_shortcode');	

/*****************************
Business Heading Section- 3
******************************/
//Function for Business Heading Section- 3
function ronby_business_heading_sec_3_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading2' => '',
	'heading1_color' => '',
	...
	$output='<!-- Section header -->
	<section class="business-heading-sec-3 p-30-0-30">
			<div class="row">
                <div class="col-12">
                    <div class="section-head b-align-center">
					'; if($heading1){ $output.='
                        <span class="small-title tp_animate_when_visible fade-bottom start_animation" data-offest="80%" data-speed="600">'.esc_attr($heading1).'</span>
					'; } $output.='	
					'; if($heading2){ $output.='
                        <h2 class="head-title tp_animate_when_visible fade-bottom start_animation mt-0 mb-0" data-offest="80%" data-speed="500" data-delay="200">'.$heading2.'
                        </h2>
					'; } $output.='	
                    </div>
                </div>
...
add_shortcode('ronby_shortcode_for_business_heading_sec_3', 'ronby_business_heading_sec_3_shortcode');

/*****************************
Shop call to action
******************************/
//Function for Shop call to action
function ronby_shop_call_to_action_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading2' => '',
	'heading1_color' => '',
	...
	$output='<section class="shop-call-to-action '.esc_attr($unique_css_class).'">
			<div class="container">
			<div class="row">
			<div class="col-md-7">
			<div class="construction-heading-section section-header-style-4 p-30-0-30 mb-0">	
			...
			'; } $output.='	
			'; if($heading2){ $output.='
				<h2 class="section-title" '; if($heading2_color){ $output.='style="color:'.esc_attr($heading2_color).'"'; } $output.='>'.$heading2.'</h2>
			'; } $output.='			
			</div>
			</div>
			<div class="col-md-5 text-center">
			<a class="button button-secondary rounded mt-45 hvr-shutter-out-horizontal" href="'.esc_url($btn_url).'" ';if($btn_margin_top) { $output.='style="margin-top:'.esc_attr($btn_margin_top).'"'; } $output.='>'.esc_attr($btn_label).'</a>
			</div>
			</div>
			</div>	
			</section>';
if($btn_text_color && $btn_bg_color){
	$output.='
	<style>
	.shop-call-to-action.'.wp_trim_words($unique_css_class).' .button{
		color:'.esc_attr($btn_text_color).' !important;
		background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.shop-call-to-action.'.wp_trim_words($unique_css_class).' .button:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		border-color: '.esc_attr($btn_hover_bg_color).' !important;
	}	
	.shop-call-to-action.'.wp_trim_words($unique_css_class).'  .hvr-shutter-out-horizontal::before{
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
}	
if($border_color){
	$output.='
	<style>
	.shop-call-to-action.'.wp_trim_words($unique_css_class).' .section-header-style-4 .section-sub-title::after{
...
add_shortcode('ronby_shortcode_for_shop_call_to_action', 'ronby_shop_call_to_action_shortcode');

/******************
Shop Feature Box
******************/
function ronby_shop_feature_box_shortcode( $atts, $content = null ) {
    extract(shortcode_atts(array(
	'bg_image'=>'',
	'heading'=>'',
	'desc'=>'',
    ...
    $output ='<!-- Start Feature Box -->
    <section class="solutions-section '.esc_attr($unique_css_class).' shop-feature-box pt-0 pb-0" >
        <div class="container-fluid">
            <div class="row row_solutions justify-content-center align-items-center ">
                <div class="col-lg-12 col-md-12 col-sm-12 col-12 col-solution p-0">
                    <div class="wrap_solution tp_animate_when_visible fade-bottom" data-offest="80%" data-speed="800" >
                        <div class="wrap_solution_img" style="background-image:url('.esc_url($bg_image).');">
						';if($heading) { $output.='
						<div class="overlay">
                            <h3>
                            '.esc_attr($heading).'
						...
						'; } $output.='
                        <a href="'.esc_url($onclick_url).'" class="solution_more" title=""><i class="fas fa-plus"></i></a>
                        
                    </div>
					';if($desc){ $output.='
                     <div class="soltion-text">
                            <p>
							'.$desc.'
							</p>
                     </div>
	...
	<!-- End Feature Box -->
'; if($hover_bg_color || $btn_bg_color || $title_color){ 
echo'<style>

'; if($hover_bg_color){ echo' 
.shop-feature-box.'.wp_trim_words($unique_css_class).' .wrap_solution_img::before{
	background-color:'.esc_attr($hover_bg_color).';
}
'; } echo'
'; if($btn_bg_color){ echo' 
.shop-feature-box.'.wp_trim_words($unique_css_class).' .solution_more{
	background-color:'.esc_attr($btn_bg_color).';	
}
'; } echo'
'; if($btn_bg_color){ echo' 
.shop-feature-box.'.wp_trim_words($unique_css_class).' .wrap_solution:hover i{
	color:'.esc_attr($btn_bg_color).';
}
'; } echo'
'; if($title_color){ echo' 
.shop-feature-box.'.wp_trim_words($unique_css_class).' .overlay h3{
	color:'.esc_attr($title_color).' !important;
}
'; } echo'
'; if($desc_color){ echo' 
.shop-feature-box.'.wp_trim_words($unique_css_class).' .soltion-text p{
	color:'.esc_attr($desc_color).' !important;
}
'; } echo'
</style>';
...
add_shortcode('ronby_shortcode_for_shop_feature_box', 'ronby_shop_feature_box_shortcode');

/*****************************
Shop Heading Section
******************************/
//Function for Shop Heading Section
function ronby_shop_heading_sec_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading2' => '',
	'heading1_color' => '',
	...
	$output='
	<section class="shop-heading-sec p-30-0-30">
		<div class="container">
			<div class="row">
                <div class="col-12">
                    <div class="section-head b-align-center text-center">
					'; if($heading1){ $output.='
                        <h1>'.esc_attr($heading1).'</h1>
					'; } $output.='	
					'; if($heading2){ $output.='
                        <h5>'.$heading2.'</h5>
					'; } $output.='	
...
add_shortcode('ronby_shortcode_for_shop_heading_sec', 'ronby_shop_heading_sec_shortcode');

/*****************************
Shop Testimonial slider
******************************/
//Function for Shop Testimonial slider
function ronby_shop_testimonial_slider_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading1_color' => '',
	'text_color' => '',
	...
	$output='
	<section class="shop-testimonial-slider '.esc_attr($unique_css_class).' p-30-0-30">
	<h2 class="text-center p-30-0-30 mt-0" '; if($heading1_color){ $output.='style="color:'.esc_attr($heading1_color).'"'; } $output.='>'.esc_attr($heading1).'</h2>
	<div class="testimonial-slider-2 owl-carousel owl-arrow-style-1 owl-nav-left-right">';
	$i=1;
	$c=0;
	while ($i<=$number_of_testimonial){
	...
	$output.='<div class="item">
						<div class="row align-items-center">
							<div class="d-sm-block  col-md-12 text-center">
							'; if($img_url){ $output.='
								<div class="avatar">
									<img src="'.esc_url($img_url).'" alt="'.esc_attr__('featured-image','ronby').'" class="m-auto">
								</div>
							'; } $output.='	
							</div>
							<div class="col-md-12 text-center">
							'; if($text){ $output.='
								<div class="mt-4" '; if($text_color){ $output.='style="color:'.esc_attr($text_color).'"'; } $output.='>
								'.esc_attr($text).'	
								</div>
							'; } $output.='	
								...
								'; } $output.='
								'; if($designation){ $output.='
									<span class="testimonial-author-role ml-0 pl-0" '; if($designation_color){ $output.='style="color:'.esc_attr($designation_color).'"'; } $output.='>- '.esc_attr($designation).'</span>
								'; } $output.='	
								</div>
							</div>
						</div>
...
if($arrow_color || $arrow_hover_bg){ 
echo'<style>

'; if($arrow_color){ echo' 
.shop-testimonial-slider.'.wp_trim_words($unique_css_class).' .owl-nav i{
	color:'.esc_attr($arrow_color).';
	border-color:'.esc_attr($arrow_color).';
}
'; } echo'
'; if($arrow_hover_bg){ echo' 
.shop-testimonial-slider.'.wp_trim_words($unique_css_class).' .owl-nav i:hover{
	background-color:'.esc_attr($arrow_hover_bg).' !important;
	border-color:'.esc_attr($arrow_hover_bg).';	
	color:#fff;
}
'; } echo'
</style>';
...
add_shortcode('ronby_shortcode_for_shop_testimonial_slider', 'ronby_shop_testimonial_slider_shortcode');

/*****************************
Shop Brands Section
******************************/
//Function for Shop Brands Section
function ronby_shop_brands_section_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'heading1' => '',
	'heading1_color' => '',
	'desc' => '',
	...
	$brand_logos = shortcode_atts(
    array(
        'images'      =>  'images',
    ), $atts );	
	$image_ids = explode(',',$brand_logos['images']);
	
	$output='
	<section class="shop-brands-sec p-30-0-30 pl-3 pr-3">
	<h2 class="text-center p-30-0-30 mt-0" '; if($heading1_color){ $output.='style="color:'.esc_attr($heading1_color).'"'; } $output.='>'.esc_attr($heading1).'</h2>
	';if($desc) { $output.='
	<div class="text-center mb-2" '; if($desc_color){ $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>
	'.$desc.'
	</div>
	'; } $output.='
	<div class="container">
	...
	foreach( $image_ids as $image_id ){
    $images = wp_get_attachment_image_src( $image_id, 'images' );
	$output.='<div class="col-md-4 text-center pt-3 pb-3">
				<img class="hvr-wobble-top" src="'.esc_url($images[$c]).'" alt="'.esc_attr__('brand-logo','ronby').'">
			</div>';
	}$c++;
	$output.='
	</div>
...
add_shortcode('ronby_shortcode_for_shop_brands_section', 'ronby_shop_brands_section_shortcode');

/*****************************
Construction Short Description
******************************/
//Function for Construction Short Description
function ronby_construction_short_desc_shortcode( $atts ) {
extract( shortcode_atts( array(
		'heading' => '',
		'desc' => '',
		'heading_color' => '',
...
$output='<section class="section-header-style-3 p-30-0-30">
			<div class="container">
			<div class="row">
			<div class="col-md-3 text-right" '; if($border_color || $padding_top || $padding_bottom || $padding_left || $padding_right) { $output.='style="'; if($border_color) { $output.='border-right:3px solid '.esc_attr($border_color).';'; } if($padding_top) { $output.='padding-top: '.esc_attr($padding_top).';'; } if($padding_bottom) { $output.='padding-bottom: '.esc_attr($padding_bottom).';'; } if($padding_left) { $output.='padding-left: '.esc_attr($padding_left).';'; } if($padding_right) { $output.='padding-right: '.esc_attr($padding_right).';'; } $output.='"'; } $output.='>
			'; if($heading){ $output.='
			<h2 class="section-title" '; if($heading_color){ $output.='style="color:'.esc_attr($heading_color).'"'; } $output.='>'.$heading.'</h2>
			'; } $output.='	
			</div>
			<div class="col-md-9" '; if($padding_top || $padding_bottom || $padding_left || $padding_right) { $output.='style="'; if($padding_top) { $output.='padding-top: '.esc_attr($padding_top).';'; } if($padding_bottom) { $output.='padding-bottom: '.esc_attr($padding_bottom).';'; } if($padding_left) { $output.='padding-left: '.esc_attr($padding_left).';'; } if($padding_right) { $output.='padding-right: '.esc_attr($padding_right).';'; } $output.='"'; }  $output.='>
			'; if($desc){ $output.='
			<p '; if($desc_color){ $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>'.$desc.'</p>
			'; } $output.='	
			</div>
...
add_shortcode('ronby_shortcode_for_construction_short_desc', 'ronby_construction_short_desc_shortcode');

/*****************************
Shop Top Categories
******************************/
//Function for Shop Top Categories
function ronby_shop_top_categories_shortcode( $atts ) {
	extract( shortcode_atts( array(
	'img_url' => '',
	'link_url' => '',
	'title' => '',
	...
	$output='<div class="shop-top-categories  '.esc_attr($unique_css_class).'" '; if($border_color) { $output.='style="border-right:1px solid '.esc_attr($border_color).'"'; } $output.='>
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-6">
						<div class="thumbnail animate-zoom">
					...
					'; if($title) { $output.='
						<h2 '; if($cat_title_color){ $output.='style="color:'.esc_attr($cat_title_color).'"'; } $output.=' class="mt-2">'.esc_attr($title).'</h2>
					'; } 
					...
					$item_url =$b['item_url'.$c.''];
					$output .='<li>';
					$output .='<a href="'.esc_url($item_url).'" '; if($list_items_color){ $output.='style="color:'.esc_attr($list_items_color).'"'; } $output.='>'.esc_attr($item_title).'</a>';										
					$output .='</li>';		
					$i++;
					}					
					$output.='	
					...
					<div class="mt-2 mb-2">
					'; if($btn_label){ $output.='
					<a href="'.esc_url($btn_url).'" class="seeall">'.esc_attr($btn_label).' <i class="fas fa-chevron-right ml-2"></i></a>
					'; } $output.='
					</div>
					</div>
				</div>
...
if($btn_text_color && $btn_bg_color){
	$output.='
	<style>
	.shop-top-categories.'.wp_trim_words($unique_css_class).' .seeall{
		color:'.esc_attr($btn_text_color).' !important;
		background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
...
if($btn_hover_text_color && $btn_hover_bg_color){
	$output.='
	<style>
	.shop-top-categories.'.wp_trim_words($unique_css_class).' .seeall:hover{
		color:'.esc_attr($btn_hover_text_color).' !important;
		background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
add_shortcode('ronby_shortcode_for_shop_top_categories', 'ronby_shop_top_categories_shortcode');

/*****************************
Shop Products Slider
******************************/
//Function for Shop Products Slider
function ronby_shop_products_slider( $atts ) {
extract( shortcode_atts( array(
	'cat_id'      => '',
	'post_per_page' => '-1',
	'order'   => 'desc',
...
$args = array(
	...
	<h2 ';if($title_color) { $output.='style="color:'.esc_attr($title_color).'"'; } $output.='>'.esc_attr($title).'</h2>
	'; } $output.='
	<div class="eltd-separator" '; if($seperator_border_color) { $output.='style="border-color:'.esc_attr($seperator_border_color).'"'; } $output.='></div>
	'; if($desc) { $output.='
	<p ';if($desc_color) { $output.='style="color:'.esc_attr($desc_color).'"'; } $output.='>'.$desc.'</p>
	'; } $output.='
	'; if($btn_label) { $output.='
	<a class="button button-secondary mt-3" href="'.esc_url($btn_url).'">'.esc_attr($btn_label).'</a>
	'; } $output.='
	</div>
	</div>
	<div class="col-md-9">
	<div class="shop-products-slider owl-carousel">';
	if ($wp_query->have_posts()) :  while ($wp_query->have_posts()) : $wp_query->the_post();
	global $product;
	global $post;
	global $redux;
	...
	$image_url = get_template_directory_uri().'/images/dummy-product-image.jpg';
	   ...
	   $i=0;
	   $average = round($product->get_average_rating());	
	$output.='<div class="item"><div class="col-md-12">
	<div class="col-md-12 nopadding">
	<article class="product-item-1 text-center" '; if($border_color){ $output.='style="border:1px solid '.esc_attr($border_color).'"'; } $output.='>';
	$ronby_product_new_badge = get_post_meta( $post->ID, 'ronby_product_new_badge', true ); 
	$ronby_product_new_badge_bg= get_post_meta( $post->ID, 'ronby_product_new_badge_bg', true );	
	...
	$output.='<div class="col-md-12 text-center pt-3 pb-3">';
	if($average){
	$output.= '<div class="stars-rating justify-content-center" data-rate="5">';
	while($i < $average){
	$output.="<span class='fas fa-star'></span>";
	...
	$output.='
	<style>
	.fitness-section-feature-products.'.wp_trim_words($unique_css_class).' .button{
	color:'.esc_attr($btn_text_color).' !important;
	background-color:'.esc_attr($btn_bg_color).' !important;
	}	
	</style>';
	...
	$output.='
	<style>
	.fitness-section-feature-products.'.wp_trim_words($unique_css_class).' .button:hover{
	color:'.esc_attr($btn_hover_text_color).' !important;
	background-color:'.esc_attr($btn_hover_bg_color).' !important;
	}	
	</style>';
...
add_shortcode('ronby_shortcode_for_shop_products_slider', 'ronby_shop_products_slider');

/*END FROM HERE ELEMENTS FUNCTIONS*/
if(class_exists('WPBakeryVisualComposerAbstract')) {
	include_once('vc_shortcodes2.php');
}