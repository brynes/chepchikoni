...
<?php 
   /* 
   *Template Name: Business Blog Thumbnail With Right Sidebar
   */ 
     
   // Blog Template start 
   if(basename(get_page_template()) == "business-blog-thumbnail-with-sidebar.php") { 
   get_header(); ?>
					...
					<?php if($get_image){?>
									<div class="col-4 col-thumbnail">
						                <a href="<?php esc_url(the_permalink())?>" class="thumbnail animate-zoom">
											<div class="blog-p-f-img h-266" style="background-image: url(<?php echo esc_url($get_image)?>); background-position: center;background-size:cover"></div>
										</a>				
									</div>
					 <?php } ?>
									<div class=" <?php if($get_image){?>col-8 col-meta<?php }else{?>col-12<?php } ?>">
