...
function ronby_scripts_basic()
{
	...
	(is_a($post, 'WP_Post') && !has_shortcode($post->post_content, 'rev_slider_vc'))) {
...
function ronby_styles_basic()
{
    ...
    /*Main Stylesheet*/
    wp_enqueue_style('ronby-main-stylesheet', get_template_directory_uri() . '/style.css', array('bootstrap','owl.carousel','owl.theme.default'));
    /*Gutenberg Only*/
    if (function_exists('the_gutenberg_project') || function_exists('wp_common_block_scripts_and_styles')) {
        wp_enqueue_style('ronby-gutenberg-additions', get_template_directory_uri() . '/gutenberg/css/gutenberg-additions.css', array('ronby-main-style-minified','ronby-sidebar-widgets','ronby-nav-only','ronby-mobile')); // Gutenberg CSS
    }
	/*layout wise shop css called*/
	if(ronby_get_option('ronby_shop_page_layout')==1){	
		wp_enqueue_style('ronby-fitness-shop-layout', get_template_directory_uri() . '/css/fitness-layout.css', array('ronby-main-style-minified','ronby-sidebar-widgets','ronby-nav-only','ronby-mobile'));			
	}elseif(ronby_get_option('ronby_shop_page_layout')==2){
		wp_enqueue_style('ronby-food-shop-layout', get_template_directory_uri() . '/css/food-layout.css', array('ronby-main-style-minified','ronby-sidebar-widgets','ronby-nav-only','ronby-mobile'));		
	}elseif(ronby_get_option('ronby_shop_page_layout')==3){
		wp_enqueue_style('ronby-fashion-shop-layout', get_template_directory_uri() . '/css/shop-layout.css', array('ronby-main-style-minified','ronby-sidebar-widgets','ronby-nav-only','ronby-mobile'));	
	}
	if (ronby_get_option('ronby_shop_page_layout') == 1 || ronby_get_option('ronby_shop_page_layout') == 2 ||
	(is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'ronby_shortcode_for_ronby_fitness_woo_products_section')) ||
	(is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'ronby_shortcode_for_business_projects_grid')) ||
...
function ronby_remove_wp_blocks()
{
    global $post;
	/*Stop WP-Block CSS*/
...
add_action('wp_enqueue_scripts', 'ronby_remove_wp_blocks', 101);
...
function ronby_add_styles_in_body() {
	/* Fonts */
...
add_action( 'wp_body_open', 'ronby_add_styles_in_body' );