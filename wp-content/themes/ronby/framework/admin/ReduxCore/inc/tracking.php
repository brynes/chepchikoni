    ...
    if ( ! class_exists( 'Redux_Tracking' ) ) {
        ...
        class Redux_Tracking {
            ...
            function tracking_request() {
                ...
                $content = '<h3>' . esc_html__( 'Help improve Our Panel', 'ronby' ) . '</h3>';
                $content .= '<p>' . esc_html__( 'Please helps us improve our panel by allowing us to gather anonymous usage stats so we know which configurations, plugins and themes to test to ensure compatibility.', 'ronby' ) . '</p>';
                $opt_arr = array(
                    'content'  => $content,
                    'position' => array( 'edge' => 'top', 'align' => 'center' )
                );
                $button2 = esc_html__( 'Allow tracking', 'ronby' );

                $function2 = 'redux_store_answer("yes","' . $nonce . '")';
                $function1 = 'redux_store_answer("no","' . $nonce . '")';

                $this->print_scripts( $id, $opt_arr, esc_html__( 'Do not allow tracking', 'ronby' ), $button2, $function2, $function1 );
            }
            ...
            function newsletter_request() {
                ...
                $content = '<h3>' . esc_html__( 'Welcome to the Redux Demo Panel', 'ronby' ) . '</h3>';
                $content .= '<p><strong>' . esc_html__( 'Getting Started', 'ronby' ) . '</strong><br>' . sprintf( esc_attr__( 'This panel demonstrates the many features of Redux.  Before digging in, we suggest you get up to speed by reviewing %1$s.', 'ronby' ), '<a href="' . 'http://' . 'docs.reduxframework.com/redux-framework/getting-started/" target="_blank">' . esc_attr__( 'our documentation', 'ronby' ) . '</a>' );
                $content .= '<p><strong>' . esc_html__( 'Redux Generator', 'ronby' ) . '</strong><br>' . sprintf( esc_attr__( 'Want to get a head start? Use the %1$s. It will create a customized boilerplate theme or a standalone admin folder complete with all things Redux (with the help of Underscores and TGM). Save yourself a headache and try it today.', 'ronby' ), '<a href="' . 'http://' . 'generate.reduxframework.com/" target="_blank">' . esc_html__( 'Redux Generator', 'ronby' ) . '</a>' );
                $content .= '<p><strong>' . esc_html__( 'Redux Extensions', 'ronby' ) . '</strong><br>' . sprintf( esc_attr__( 'Did you know we have extensions, which greatly enhance the features of Redux?  Visit our %1$s to learn more!', 'ronby' ), '<a href="' . 'http://' . 'reduxframework.com/extensions/" target="_blank">' . esc_html__( 'extensions directory', 'ronby' ) . '</a>' );
                $content .= '<p><strong>' . esc_html__( 'Like Redux?', 'ronby' ) . '</strong><br>' . sprintf( esc_attr__( 'If so, please %1$s and consider making a %2$s to keep development of Redux moving forward.', 'ronby' ), '<a target="_blank" href="' . 'http://' . 'wordpress.org/support/view/plugin-reviews/redux-framework">' . esc_html__( 'leave us a favorable review on WordPress.org', 'ronby' ) . '</a>', '<a href="' . 'https://' . 'www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=N5AD7TSH8YA5U" target="_blank">' . esc_html__( 'donation', 'ronby' ) . '</a>' );
                $content .= '<p><strong>' . esc_html__( 'Newsletter', 'ronby' ) . '</strong><br>' . esc_html__( 'If you\'d like to keep up to with all things Redux, please subscribe to our newsletter', 'ronby' ) . ':</p>';
                $content .= '<form action="http://news.redux.io/subscribe" method="POST" target="_blank" accept-charset="utf-8" class="validate">
                                <p style="text-align: center;">
                                    <label for="email">' . esc_html__('Email Address', 'ronby') . '</label>
                                    <input type="email" name="email" class="required email" id="email"/>
                                    <input type="hidden" name="list" value="9K1qDRvB8Ux0DqpEoQSEPA"/>
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    <input type="submit" class="button button-primary" name="submit" value="' . esc_html__( 'Subscribe', 'ronby' ) . '" id="submit"/>
                                    </p>
                            </form>';
                ...
                $this->print_scripts( $id, $opt_arr, esc_html__( 'Close', 'ronby' ), false, '', $function1 );
            }
