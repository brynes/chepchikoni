        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'ronby' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'ronby' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    if ( $this->args['dev_mode'] == true || Redux_Helpers::isLocalHost() == true ) {
                        ...
                        if ( ! isset ( $GLOBALS['redux_notice_check'] ) || $GLOBALS['redux_notice_check'] == 0 ) {
                            ...
                            $params = array(
                                'dir_name'    => 'notice',
								'server_file' => 'core/redux_notice.json',
                                'interval'    => 3,
            ...
            private function _internationalization() {
                 ...
                 * @param string     'ronby'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'ronby' );
                //
                ...
                $loaded = load_plugin_textdomain( 'ronby', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'ronby', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'ronby', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'ronby' );
                    $mofile = dirname( __FILE__ ) . '/languages/redux-framework-' . $locale . '.mo';
                    load_textdomain( 'ronby', $mofile );
                }
            }
...
//                    }
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'ronby' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'ronby' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_attr__( 'moving the mouse over', 'ronby' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_attr__( 'clicking', 'ronby' );
                    }

                    // Construct message
                    $msg = sprintf( esc_attr__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'ronby' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_attr__( 'Hints', 'ronby' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_attr__( 'Default', 'ronby' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_attr__( 'Invalid security credential.  Please reload the page and try again.', 'ronby' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_attr__( 'Invalid user capability.  Please reload the page and try again.', 'ronby' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( $do_reload || ( isset ( $values['defaults'] ) && ! empty ( $values['defaults'] ) ) || ( isset ( $values['defaults-section'] ) && ! empty ( $values['defaults-section'] ) ) || ( isset ( $values['import_code'] ) && ! empty ($values['import_code']) ) || ( isset ( $values['import_link'] ) && ! empty ($values['import_link']) ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_attr__( 'Your panel has no fields. Nothing to save.', 'ronby' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
